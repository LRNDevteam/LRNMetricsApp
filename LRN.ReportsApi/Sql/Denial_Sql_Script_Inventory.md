...
| `DenialWorkflow_MasterValues_Setup.sql` | Master database | Not merged | The seven workflow master lists (Classification, Coverage Status, ICD Compliance, Denial Validity, Action Category, SLA, Priority) maintained on the admin **Master Values** screen. Creates `DenialMapperLookupMaster` / `DenialMapperActionCategoryMaster` if missing, adds audit columns, seeds empty tables only. Re-runnable and non-destructive. Optional: the Reports API applies the same changes on first use. |
| `DenialMapper_MasterData_Setup.sql` | One-off master data normalisation | Not merged | **Do not re-run** once admins use Master Values: its MERGE re-activates and re-inserts every value in its list, undoing admin changes. Use `DenialWorkflow_MasterValues_Setup.sql` for new environments. |
...
| `DenialSummary_Setup.sql` | Lab database | Not merged | Denial Summary observations (`DenialSummaryObservation`) and weekly/monthly/on-demand Excel snapshots (`DenialSummarySnapshot`). Re-runnable and non-destructive. Optional: the Reports API creates the same objects on first use. |
