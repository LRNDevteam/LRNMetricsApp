/* ==============================================================================
   LRN Payer Policy Mapper - CONSOLIDATED DEPLOYMENT SCRIPT
   Target database: LRNMaster (ConnectionStrings:DefaultConnection)
   Generated: 2026-07-16 by consolidating the individual scripts
   in LRN.ReportsApi\Sql in strict dependency order.

   PREREQUISITES (must already exist - NOT created here):
     - dbo.PayerPolicyInsuranceMaster
     - dbo.LabInsuranceMaster
     - dbo.MenuItems / dbo.Roles / dbo.UserRoleMenu (for the menu item in Section 4)

   EXECUTION ORDER (this file runs them top to bottom):
     1. Payer_Matching_Reference_Tables_DDL_and_Seed.sql  ->  Reference / rules-engine tables + seed data
     2. PayerMaster_Workflow_Setup.sql  ->  Payer Master workflow infrastructure
     3. 001_payer_mapper_additions.sql  ->  LabInsuranceMaster mapping columns + PendingMatchCandidates + PayerMatchAudit
     4. 002_payer_mapper_service_audit.sql  ->  PayerMapperRun (service run audit) + RunId + menu item
     5. 003_payer_mapper_run_scope.sql  ->  Run scope
     6. 004_payer_mapping_audit_fields.sql  ->  Mapping audit fields + United Healthcare family rule

   IDEMPOTENCY:
     - Section 1 is DESTRUCTIVE (DROP + recreate of the rules tables). Run once on a
       fresh install. On an existing database, comment it out to preserve learned
       PayerAlias rows and rule edits; Sections 2-6 are safe to re-run.
     - Sections 2-6 are idempotent (existence-checked ADDs / CREATEs).
   ============================================================================== */


/* ==============================================================================
   SECTION 1 - Reference / rules-engine tables + seed data
   Source: Payer_Matching_Reference_Tables_DDL_and_Seed.sql
   USStateCode, PayerFamilyRule, StateBrandMapping, ProgramTypeRule, PlanNetworkTypeCode, ProductLineRule, PayerAlias (seed).
   WARNING: this section DROPS and recreates these tables - it is DESTRUCTIVE on re-run (it wipes PayerAlias rows learned by the feedback loop and any business edits to the rule patterns). Run it once on a fresh install; SKIP this section when re-applying to an existing environment.
   ============================================================================== */
GO
...
/* ==============================================================================
   SECTION 2 - Payer Master workflow infrastructure
   Source: PayerMaster_Workflow_Setup.sql
   PayerMasterApprovalRequests, PayerMasterAuditTrail, PayerMasterNotifications (used by the mapper for review notifications + import audit), and the Global Payer ID sequence. Idempotent.
   ============================================================================== */
GO
...
/* ==============================================================================
   SECTION 3 - LabInsuranceMaster mapping columns + PendingMatchCandidates + PayerMatchAudit
   Source: 001_payer_mapper_additions.sql
   Adds MappingStatus / MappedBy / LastEvaluatedOn to dbo.LabInsuranceMaster (with backfill) and creates dbo.PendingMatchCandidates + dbo.PayerMatchAudit. Idempotent.
   ============================================================================== */
GO
...
/* ==============================================================================
   SECTION 4 - PayerMapperRun (service run audit) + RunId + menu item
   Source: 002_payer_mapper_service_audit.sql
   dbo.PayerMapperRun, RunId column on dbo.PayerMatchAudit, and the Payer Service Audit menu item. Idempotent.
   ============================================================================== */
GO
...
/* ==============================================================================
   SECTION 5 - Run scope
   Source: 003_payer_mapper_run_scope.sql
   Adds Scope to dbo.PayerMapperRun (UnmappedPending / AllUnmapped / All). Idempotent.
   ============================================================================== */
GO
...
/* ==============================================================================
   SECTION 6 - Mapping audit fields + United Healthcare family rule
   Source: 004_payer_mapping_audit_fields.sql
   Adds MappedSource / MappedOn to dbo.LabInsuranceMaster and appends 'UNITED HEALTHCARE' to the UHC PayerFamilyRule pattern. Idempotent.
   ============================================================================== */
GO
