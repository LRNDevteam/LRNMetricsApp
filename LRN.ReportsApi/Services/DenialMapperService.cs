...
public sealed class SqlDenialMapperRepository : IDenialMapperRepository
{
    ...
    public async Task<DenialCodeMasterImportResult> ImportSuperMasterAsync(Stream stream,string fileName,string user,string role,CancellationToken ct)
    {
        ...
        foreach(var row in sheet.Rows(firstDataRow,sheet.LastRowUsed()?.RowNumber()??firstDataRow)){var rowNo=row.RowNumber();var q=new DenialMapperSaveRequest{DenialCode=Cell(row,"Denial Code","DenialCode"),DenialDescription=Cell(row,"Denial Description","Description"),DenialClassification=Cell(row,"Denial Classification","Classification"),CoverageStatus=Cell(row,"Coverage Status","CoverageStatus"),ICDComplianceStatus=Cell(row,"ICD Compliance Status","ICD Compliance","ICDComplianceStatus"),DenialValidity=Cell(row,"Denial Validity","DenialValidity"),ActionCode=Cell(row,"Action Code","ActionCode"),ActionCategory=Cell(row,"Action Category","ActionCategory"),Task=Cell(row,"Task"),RecommendedAction=Cell(row,"Recommended Action","RecommendedAction"),SLA=Cell(row,"SLA","SLA Days","SLA (Days)","SLADays"),Priority=Cell(row,"Priority")};if(string.IsNullOrWhiteSpace(q.DenialCode)&&string.IsNullOrWhiteSpace(q.ActionCode))continue;
            // Only Denial Code and Action Code identify the mapping and are required; ActionCategory/
            // Task/RecommendedAction/SLA/Priority are legitimately blank for some codes in the source
            // workbook (a "no action defined yet" row is still a row to import, not an error) - a row
            // must never be silently dropped over an optional column being empty.
            if(string.IsNullOrWhiteSpace(q.DenialCode)||string.IsNullOrWhiteSpace(q.ActionCode)){errors.Add($"Row {rowNo}: Denial Code and Action Code are required.");continue;}records.Add(q);}
        ...
        // inserted/updated alone can't tell a genuine refresh of a pre-existing mapping apart from
        // two rows in THIS file sharing the same (DenialCode, Classification, CoverageStatus,
        // ICDComplianceStatus) key - that key is the table's own unique constraint, so only the last
        // row per key can end up as its own row. seenKeys tracks which key this batch has already
        // written, so the duplicate case gets reported honestly instead of counted as a plain update.
        var inserted=0;var updated=0;var mergedDuplicateCount=0;var seenKeys=new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        await using var c=Open();await c.OpenAsync(ct);await using var tx=(SqlTransaction)await c.BeginTransactionAsync(ct);try{foreach(var q in records){var key=$"{q.DenialCode.Trim()}{q.DenialClassification?.Trim()}{q.CoverageStatus?.Trim()}{q.ICDComplianceStatus?.Trim()}";var seenBefore=!seenKeys.Add(key);const string existsSql="SELECT Id FROM dbo.DenialMapperSuperMaster WHERE DenialCode=@Code AND ISNULL(DenialClassification,'')=ISNULL(@Class,'') AND ISNULL(CoverageStatus,'')=ISNULL(@Coverage,'') AND ISNULL(ICDComplianceStatus,'')=ISNULL(@Icd,'') AND IsActive=1";long? id;await using(var find=new SqlCommand(existsSql,c,tx)){find.Parameters.AddWithValue("@Code",q.DenialCode);find.Parameters.AddWithValue("@Class",Db(q.DenialClassification));find.Parameters.AddWithValue("@Coverage",Db(q.CoverageStatus));find.Parameters.AddWithValue("@Icd",Db(q.ICDComplianceStatus));var found=await find.ExecuteScalarAsync(ct);id=found is null?null:Convert.ToInt64(found);}const string insert="INSERT dbo.DenialMapperSuperMaster(DenialCode,DenialDescription,DenialClassification,CoverageStatus,ICDComplianceStatus,DenialValidity,ActionCode,ActionCategory,Task,RecommendedAction,SLA,Priority,CreatedBy,ModifiedBy) VALUES(@Code,@Description,@Class,@Coverage,@Icd,@Validity,@ActionCode,@ActionCategory,@Task,@Recommended,@Sla,@Priority,@User,@User)";const string update="UPDATE dbo.DenialMapperSuperMaster SET DenialDescription=@Description,DenialValidity=@Validity,ActionCode=@ActionCode,ActionCategory=@ActionCategory,Task=@Task,RecommendedAction=@Recommended,SLA=@Sla,Priority=@Priority,ModifiedBy=@User,ModifiedOn=SYSUTCDATETIME() WHERE Id=@Id";await using var cmd=new SqlCommand(id.HasValue?update:insert,c,tx);AddMaster(cmd,q,user);if(id.HasValue){cmd.Parameters.AddWithValue("@Id",id.Value);if(seenBefore)mergedDuplicateCount++;else updated++;}else inserted++;await cmd.ExecuteNonQueryAsync(ct);}await Audit(c,tx,"Super Master Uploaded",null,null,"All",null,null,$"{records.Count} rows from {fileName}",user,role,ct);await tx.CommitAsync(ct);}catch{await tx.RollbackAsync(CancellationToken.None);throw;}
        var message=mergedDuplicateCount>0
            ? $"Processed {records.Count} row(s): {inserted} inserted, {updated} updated, {mergedDuplicateCount} merged as duplicates (same Denial Code + Classification + Coverage + ICD Compliance - only the last row in the file was kept). {errors.Count} row(s) skipped (missing Denial Code or Action Code)."
            : $"Processed {records.Count} row(s): {inserted} inserted, {updated} updated. {errors.Count} row(s) skipped (missing Denial Code or Action Code).";
        return new(){InsertedCount=inserted,UpdatedCount=updated,MergedDuplicateCount=mergedDuplicateCount,FailedCount=errors.Count,Errors=errors,Message=message};
    }
