...
SELECT
    ...
    {SelectString(cols, "WorkFlowStatus")},
...
ORDER BY {OrderBy(cols, "DueDate", "TaskID")};";
			...
			while (await reader.ReadAsync(cancellationToken))
			{
				items.Add(new DenialRecord
				{
					...
					WorkFlowStatus = GetString(reader, "WorkFlowStatus"),
	...
	/// <summary>
	/// Reads the pre-aggregated dbo.DenialInsight rows for the lab's current run. Falls back to the
	/// task-board derived insights when the table is absent or holds nothing for that run.
	/// </summary>
	...
	{
		var currentRunId = await GetCurrentRunIdAsync(labId, cancellationToken);
		var cacheKey = $"{InsightTableCacheKey(labId)}:{currentRunId ?? "all"}";
		...
		var rows = await _cache.GetOrCreateAsync(cacheKey, async entry =>
		{
			...
			await using var connection = await OpenLabConnectionAsync(labId, cancellationToken);
			if (!await TableExistsAsync(connection, "dbo", "DenialInsight", cancellationToken))
			{
				return (IReadOnlyList<DenialInsightRecord>)Array.Empty<DenialInsightRecord>();
			}

			var cols = await GetTableColumnsAsync(connection, "dbo", "DenialInsight", cancellationToken);
			var where = BuildScopedWhere(cols, currentRunId);
			var orderBy = cols.Contains("TotalBalance")
				? "TRY_CONVERT(decimal(18, 2), [TotalBalance]) DESC"
				: cols.Contains("DenialCodes") ? "[DenialCodes]" : "(SELECT NULL)";

			var sql = $@"
SELECT
    {SelectString(cols, "DenialCodes")},
    {SelectString(cols, "Descriptions")},
    {SelectInt(cols, "NoOfDenialCount")},
    {SelectInt(cols, "NoOfClaimsCount")},
    {SelectDecimal(cols, "TotalBalance", 2)},
    {SelectString(cols, "HighImpactInsurance")},
    {SelectDecimal(cols, "InsuranceBalance", 2)},
    {SelectDecimal(cols, "ImpactPercentage", 2)},
    {SelectString(cols, "ActionCategory")},
    {SelectString(cols, "ActionCode")},
    {SelectString(cols, "Action")},
    {SelectString(cols, "Task")},
    {SelectString(cols, "Feedback")},
    {SelectString(cols, "Responsibility")},
    {SelectString(cols, "AssignedTo")},
    {SelectString(cols, "ResponsibilityReviewer")},
    {SelectDate(cols, "DiscussionDate")},
    {SelectString(cols, "ETA")},
    {SelectString(cols, "LabName")},
    {SelectInt(cols, "LabId")},
...
ORDER BY {orderBy};";
			...
			while (await reader.ReadAsync(cancellationToken))
			{
				var reviewer = GetString(reader, "ResponsibilityReviewer");
				var totalBalance = GetNullableDecimal(reader, "TotalBalance") ?? 0m;
				var insuranceBalance = GetNullableDecimal(reader, "InsuranceBalance") ?? 0m;
				var storedImpact = GetNullableDecimal(reader, "ImpactPercentage");

				items.Add(new DenialInsightRecord
				{
					DenialCodes = GetString(reader, "DenialCodes"),
					Descriptions = GetString(reader, "Descriptions"),
					NoOfDenialCount = GetNullableInt(reader, "NoOfDenialCount") ?? 0,
					NoOfClaimsCount = GetNullableInt(reader, "NoOfClaimsCount") ?? 0,
					TotalBalance = totalBalance,
					HighImpactInsurance = GetString(reader, "HighImpactInsurance"),
					InsuranceBalance = insuranceBalance,
					ImpactPercentage = storedImpact ?? (totalBalance == 0 ? 0 : decimal.Round((insuranceBalance / totalBalance) * 100m, 2)),
					ActionCategory = NormalizeMappedValue(GetString(reader, "ActionCategory")),
					ActionCode = GetString(reader, "ActionCode"),
					...
					Responsibility = GetString(reader, "Responsibility"),
					ResponsibilityReviewer = string.IsNullOrWhiteSpace(reviewer) ? GetString(reader, "AssignedTo") : reviewer,
					DiscussionDate = GetNullableDateTime(reader, "DiscussionDate"),
					ETA = GetString(reader, "ETA"),
					LabName = GetString(reader, "LabName"),
					LabId = GetNullableInt(reader, "LabId") ?? labId,
			...
			return (IReadOnlyList<DenialInsightRecord>)items;
		}) ?? Array.Empty<DenialInsightRecord>();

		return rows.Count > 0 ? rows : await GetInsightsByLabAsync(labId, cancellationToken);
	}
...
SELECT MatchedRows = (SELECT COUNT(1) FROM #MatchedRows);";
	...
	public async Task<int> AssignReviewerByInsightAsync(int labId, string denialCode, string payerName, string reviewerUserName, string? runId, CancellationToken cancellationToken = default)
	{
		...
		// DenialInsight.HighImpactInsurance may hold either the raw or the normalized payer name,
		// depending on the loader, so match a task against whichever column agrees.
		var payerPredicates = new List<string>();
		if (cols.Contains("PayerNameNormalized")) payerPredicates.Add("LTRIM(RTRIM(ISNULL(CONVERT(nvarchar(255), [PayerNameNormalized]), ''))) = @PayerName");
		if (cols.Contains("PayerName")) payerPredicates.Add("LTRIM(RTRIM(ISNULL(CONVERT(nvarchar(255), [PayerName]), ''))) = @PayerName");
		if (payerPredicates.Count == 0) return 0;
