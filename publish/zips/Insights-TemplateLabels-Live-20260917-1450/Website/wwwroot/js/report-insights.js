...
(function () {
    ...
    function sanitizeRichHtml(s) {
        ...
        wrap.innerHTML = s;
        wrap.querySelectorAll("script,iframe,object,embed,link,meta,form").forEach(el => el.remove());
        wrap.querySelectorAll("*").forEach(el => {
            [...el.attributes].forEach(a => {
                const n = a.name.toLowerCase();
                if (n.startsWith("on") || n === "src" || n === "srcset") el.removeAttribute(a.name);
                if (n === "href") {
            ...
            if (el.getAttribute("style")) {
                el.setAttribute("style", String(el.getAttribute("style")).replace(/expression|url\s*\(|javascript/gi, ""));
            }
        });
    ...
    function reportLayout() {
        ...
        const base = r.includes("collection")
            ? {
                risk: "Risk", party: "Responsible Party", insights: "Insights",
                claims: "# of Cases", charge: "Total Billed",
                action: "Action / Solution / Suggestion", feedback: "Feedback / Response",
                owner: "Response By", discuss: "Discussion Date", eta: "ETA",
                closed: "Closed Date", status: "Status",
            ...
            ? {
                risk: "Risk", party: "Responsible Party", insights: "Insights",
                claims: "# of Claims", charge: "Expected Reimbursement ($)",
                action: "Action / Solution / Suggestions", feedback: "Feedback / Response",
                owner: "Responsibility", discuss: "Discussion Date", eta: "ETA",
                closed: "Closed Date", status: "Status",
            ...
            : {
                risk: "Risk", party: "Responsible Party", insights: "Insights",
                claims: "# of Claims", charge: "Total Charge",
                action: "Action / Solution / Suggestions", feedback: "Feedback / Response",
                owner: "Responsibility", discuss: "Discussion Date", eta: "ETA",
                closed: "Closed Date", status: "Status",
                footer: "Previously Analysed Data - Pending Items",
        ...
        const nameFor = key => {
            const cols = tplColumns();
            const hit = cols.find(c => String(fieldKey(c)).toLowerCase() === String(key).toLowerCase());
            const n = hit && String(colName(hit) || "").trim();
            return n || "";
        };
        return {
            risk: nameFor("Risk") || base.risk,
            party: nameFor("ResponsibleParty") || base.party,
            insights: nameFor("Insights") || base.insights,
            claims: nameFor("NoOfClaims") || base.claims,
            charge: nameFor("TotalCharge") || base.charge,
            action: nameFor("ActionSolution") || base.action,
            feedback: nameFor("FeedbackResponse") || base.feedback,
            owner: nameFor("Responsibility") || base.owner,
            discuss: nameFor("DiscussionDate") || base.discuss,
            eta: nameFor("ETA") || base.eta,
            closed: nameFor("ClosedDate") || base.closed,
            status: nameFor("Status") || base.status,
            footer: base.footer,
            footerSub: base.footerSub
        };
    }
    ...
    function colName(col) {
        return (col && (col.columnName || col.ColumnName)) || "";
    }
    function fieldKey(col) {
        const k = String((col && (col.fieldKey || col.FieldKey)) || "").trim();
        if (k) return k;
        const n = String(colName(col) || "").toLowerCase();
        ...
        if (n.includes("charge") || n.includes("bill") || n.includes("reimbursement") || n.includes("balance")) return "TotalCharge";
    ...
    function bindClaimsInput(input) {
        if (!input) return;
        const apply = () => { input.value = formatClaims(input.value); };
    ...
    function openDeleteConfirm(noteId) {
        ...
        if (el.deleteEntry) el.deleteEntry.textContent = row && row.displayNo != null ? ("#" + row.displayNo) : ("#" + noteId);
    ...
    function buildTableHtml(opts) {
        ...
        const colCount = float ? 13 : 14;
        const thead = `<thead><tr>
            <th>#</th><th>${esc(L.risk)}</th><th>${esc(L.party)}</th><th class="ri-insights">${esc(L.insights)}</th>
            <th>${esc(L.claims)}</th><th>${esc(L.charge)}</th>
            <th class="ri-action">${esc(L.action)}</th>
            <th>${esc(L.feedback)}</th><th>${esc(L.owner)}</th>
            <th>${esc(L.discuss)}</th><th>${esc(L.eta)}</th><th>${esc(L.closed)}</th><th>${esc(L.status)}</th>${float ? "" : "<th></th>"}
        </tr></thead>`;
        ...
        return `<table class="ri-table">${thead}<tbody>${rows.map(n => {
            ...
            return `<tr>
                <td>${esc(n.displayNo ?? "")}${carry}</td>
                <td class="ri-risk-${esc(risk)}">${esc(risk)}</td>
                <td>${esc(n.responsibleParty || "")}</td>
                <td class="ri-insights"><div class="ri-insights-html">${sanitizeRichHtml(n.insights || "")}</div></td>
            ...
            <div class="ri-detail-layout">
                    ...
                    <section class="ri-info-card">
                        ...
                        <div class="ri-info-body">
                            <div class="ri-field-grid" style="margin-bottom:9px;">
                                <div class="ri-field">
                                    <label for="ri_Risk">${esc(L.risk)}</label>
                                ...
                                <div class="ri-field">
                                    <label for="ri_ResponsibleParty">${esc(L.party)}</label>
                                ...
                                <div class="ri-field">
                                    ...
                                    <input id="ri_NoOfClaims" data-key="NoOfClaims" type="text" inputmode="numeric" value="${esc(formatClaims(row && row.noOfSamples))}" ${roAttr} />
                                </div>
                                <div class="ri-field">
                                    <label for="ri_Status">${esc(L.status)}</label>
                    ...
                    <section class="ri-info-card">
                        ...
                        <div class="ri-info-body">
                            ...
                            <div class="ri-field-grid">
                                <div class="ri-field">
                                    <label for="ri_FeedbackResponse">${esc(L.feedback)}</label>
                ...
                <div class="ri-detail-col">
                    <section class="ri-info-card">
                        ...
                        <div class="ri-info-body">
                            <div class="ri-field-grid">
                                <div class="ri-field"><label for="ri_DiscussionDate">${esc(L.discuss)}</label><input id="ri_DiscussionDate" data-key="DiscussionDate" type="date" value="${esc(isoDate(row && row.discussionDate))}" ${roAttr} /></div>
                                <div class="ri-field"><label for="ri_ETA">${esc(L.eta)}</label><input id="ri_ETA" data-key="ETA" type="date" value="${esc(isoDate(row && row.eta))}" ${roAttr} /></div>
                                <div class="ri-field"><label for="ri_ClosedDate">${esc(L.closed)}</label><input id="ri_ClosedDate" data-key="ClosedDate" type="date" value="${esc(isoDate(row && row.closedDate))}" ${roAttr} /></div>
    ...
    function readForm() {
        ...
        // Empty strings must become null — System.Text.Json rejects "" for DateTime?
        // and then [FromBody] binds as null → "Missing payload."
        const dateOrNull = v => {
        ...
        return {
            ...
            weekRangeStart: dateOrNull((editing && editing.weekRangeStart) || ctx.weekStart) || new Date().toISOString().slice(0, 10),
            weekRangeEnd: dateOrNull((editing && editing.weekRangeEnd) || ctx.weekEnd || ctx.weekStart) || new Date().toISOString().slice(0, 10),
            riskCode: riskCode(get("Risk") || "Yellow"),
            responsibleParty: get("ResponsibleParty") || null,
            insights: get("Insights") || null,
            noOfSamples: nClaims,
            totalCharge: Number.isFinite(nCharge) ? nCharge : null,
            dataLink: editing && editing.dataLink ? editing.dataLink : null,
            actionSolution: get("ActionSolution") || null,
            feedbackResponse: get("FeedbackResponse") || null,
            responsibility: get("Responsibility") || null,
            discussionDate: dateOrNull(get("DiscussionDate")),
            eta: dateOrNull(get("ETA")),
    ...
    async function openHistory(noteId, fromArchive) {
        ...
        if (el.historySummary) {
            el.historySummary.innerHTML = `
                <div class="ri-ctx-card"><div class="ri-ctx-label">Note</div><div class="ri-ctx-value">${esc(note ? ("#" + (note.displayNo || note.entryNo || noteId)) : noteId)}</div></div>
    ...
    async function openArchive() {
        ...
        try {
            ...
            </tr></thead><tbody>${archived.map((n, i) => `<tr>
                <td>${i + 1}</td>
    ...
    function fillTemplates() {
        const prev = el.tpl ? String(el.tpl.value || "") : "";
        el.tpl.innerHTML = templates.map(t => `<option value="${t.templateId}">${esc(t.templateName || t.TemplateName || "")}</option>`).join("")
            || `<option value="">Key Insights & Highlights</option>`;
        if (templates.length >= 1) {
            selectedTpl = templates.find(t => String(t.templateId || t.TemplateId) === prev) || templates[0];
            el.tpl.value = String(selectedTpl.templateId || selectedTpl.TemplateId || "");
        } else {
    ...
    async function refreshTemplates() {
        ...
        } catch { /* keep last known templates */ }
        fillTemplates();
    }
    ...
    async function loadRows() {
        ...
        rows.forEach((n, i) => { n.displayNo = i + 1; });
        renderRows();
    }
    ...
    function setOpen(open) {
        ...
        if (open) {
            ensureLoaded().then(() => refreshTemplates().then(() => renderRows()));
        }
    }
    setOpen(false);
    if (toggle) {
