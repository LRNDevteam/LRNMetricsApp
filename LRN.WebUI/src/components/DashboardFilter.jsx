...
export default function DashboardFilter({ filter, setFilterValue, clearFilter, reviewers, options, visibleFilters }) {
  ...
  return <div className="lrn-card filter-card"><div className="filter-row">
    ...
    {show('assignedUserId') && <SearchableMultiSelect label="Assigned User" value={filter.assignedUserId} onChange={v => setFilterValue('assignedUserId', v)} options={options?.assignedUsers || []} placeholder="All assigned users" />}
    {show('assignedDuration') && <div className="filter-search"><label>Assigned Duration</label><select value={filter.assignedDuration || ''} onChange={e => setFilterValue('assignedDuration', e.target.value)}><option value="">All</option><option>Assigned Today</option><option>Assigned This Week</option><option>Assigned This Month</option><option>Assigned More Than This Month</option></select></div>}
