...
export default function ClaimAssignmentPage({ data, reviewers, selected, setSelected, bulkReviewer, setBulkReviewer, loadClaimTasks, claimTasks, expandedClaim, assignClaims, changePage, labId, currentUser, canAssign = false, readOnlyWorkflow = false, taskView = 'unassigned', setTaskView = () => {}, tabCounts = {}, openEscalationResponse = () => {}, openVerification = () => {}, setMessage = () => {}, onDownloadTab = () => {}, onDownloadTemplate = () => {}, onCsvUploaded = () => {}, exportBusy = false, exportStatusText = '', loading = false }) {
    ...
    <div className={`claim-split-shell ${activeClaim ? 'drawer-open' : ''} ${loading ? 'claims-loading' : ''}`}>
      {!activeClaim && <section className="claim-list-pane thin-list-border full">
        ...
        <div className="claim-list-toolbar">
          ...
          })}<button type="button" className="claim-tab-download" onClick={onDownloadTab} disabled={exportBusy} title={exportStatusText || `Download ${activeTab.label} claims using current filters`}><i className="bi bi-download" />{exportBusy ? 'Export running' : `Download ${activeTab.label}`}</button>{!readOnlyWorkflow && <ClaimCsvUpload labId={labId} setMessage={setMessage} onUploaded={onCsvUploaded} onDownloadTemplate={onDownloadTemplate} templateBusy={exportBusy} />}</div>
        </div>
