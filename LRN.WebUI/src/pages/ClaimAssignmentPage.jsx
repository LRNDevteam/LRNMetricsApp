...
export default function ClaimAssignmentPage({ data, reviewers, selected, setSelected, bulkReviewer, setBulkReviewer, loadClaimTasks, claimTasks, expandedClaim, assignClaims, changePage, changePageSize = () => {}, pageSize = 50, labId, currentUser, currentUserRole = '', canAssign = false, readOnlyWorkflow = false, taskView = 'unassigned', setTaskView = () => {}, tabCounts = {}, openEscalationResponse = () => {}, openVerification = () => {}, setMessage = () => {}, onDownloadTemplate = () => {}, onCsvUploaded = () => {}, exportBusy = false, loading = false, filter = {}, setFilterValue = () => {}, assignedUsers = [] }) {
  ...
  // "External Response" (AR Manager) is a response-review view: it renders as the standard
  // claim + line drill-down (like every other tab) with a "Review Response" action on each line,
  // plus the shared Notes / Documents / History drawer -- not the separate flat escalation page.
  const isResponseView = taskView === 'externalResponse' || taskView === 'escalationResponse';
  const isEscalatedView = taskView === 'internalEscalation' || taskView === 'externalEscalation' || taskView === 'escalations' || isResponseView;
  ...
  // Use the viewer's actual role so Client/Account Managers get their own tab set (and the unified
  // claim+line view) instead of falling back to the AR Reviewer tabs. Escalation tabs render
  // inline here (no route to the separate escalation page).
  const claimTabs = useMemo(() => [
    ...getQueuesForRole(currentUserRole),
    ...(canAssign ? [{ key: 'verification', label: 'Verification Claim', route: openVerification }] : [])
  ], [currentUserRole, canAssign, openVerification]);
  ...
  async function openEscalation(target) {
    // AR Manager/Admin can escalate + respond; Client/Account Managers can RESPOND to an external
    // escalation that was routed to them (they cannot create/escalate). Everyone else is blocked.
    const externalResponding = externalManager && target?.mode === 'respond';
    if (!canAssign && !externalResponding) return setMessage({ type: 'warning', text: 'Only Admin and AR Manager users can escalate claims/tasks.' });
  ...
  async function submitEscalationPopup() {
    // Allow Client/Account Managers through only for the response path (mode === 'respond').
    if (!canAssign && !(externalManager && escalationPopup?.mode === 'respond')) return;
    ...
    try {
      if (responseMode) {
        ...
        const resolveResult = await denialWorkflowService.resolveEscalation({
          ...
          // Responder role drives the response label (Account Manager Response / Client Response)
          // and the External Response queue detection.
          actionByRole: currentUserRole || ''
        });
      ...
      {!activeClaim && <section className="claim-list-pane thin-list-border full">
        ...
        <div className="claim-list-toolbar">
          ...
          <div className="claim-tab-row">{claimTabs.map(t => {
            ...
            // Highlight alert tabs (e.g. the Client/Account Manager "Respond Escalation" tab) when
            // they have pending items, so the manager sees there is work to respond to.
            const hasAlert = t.alert && Number(count || 0) > 0;
            return <button key={t.key} type="button" className={`claim-tab ${active ? 'active' : ''} ${active && loading ? 'is-loading' : ''} ${hasAlert ? 'has-alert' : ''}`} title={t.hint} onClick={() => t.route ? t.route() : setTaskView(t.key)} disabled={active && loading}><span>{t.label}</span>{active && loading ? <i className="bi bi-arrow-repeat claim-tab-spinner" aria-label="Loading claims" /> : <b>{tabCountText(count)}</b>}</button>;
