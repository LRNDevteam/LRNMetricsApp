...
export default function ClaimAssignmentPage({ data, reviewers, selected, setSelected, bulkReviewer, setBulkReviewer, loadClaimTasks, claimTasks, expandedClaim, assignClaims, changePage, labId, currentUser, canAssign = false, readOnlyWorkflow = false, taskView = 'unassigned', setTaskView = () => {}, tabCounts = {}, openEscalationResponse = () => {}, openVerification = () => {}, setMessage = () => {} }) {
  ...
  async function saveNote() {
    ...
    setBusy(true);
    try {
      const line = selectedLineTarget(noteLineTarget, notePopup.claimId);
      await denialWorkflowService.saveNote({ labId, claimId: notePopup.claimId, taskId: line.taskId || null, cptCode: line.cptCode || null, noteLevel: 'Claim', noteText, status: noteStatus, nextFollowUpDate: noteFollowUpDate || null, createdBy: currentUser });
      setNoteText('');
  ...
  async function openClaimDocuments(claimId, targetKey = '') {
    setDocPopup({ claimId, title: `Claim Documents — ${claimId}`, options: claimLineOptions(claimId) });
  ...
  async function uploadDocuments() {
    ...
    setBusy(true);
    try {
      const line = selectedLineTarget(docLineTarget, docPopup.claimId);
  ...
  const tableColSpan = canAssign ? 16 : 14;
  const activeClaim = useMemo(() => items.find(r => getClaimId(r) === expandedClaim) || null, [items, expandedClaim]);
  const activeClaimId = activeClaim ? getClaimId(activeClaim) : '';
  ...
  function toggleClaim(claimId) {
    if (expandedClaim !== claimId) resetDrawerState();
  ...
  return <>
    <div className={`claim-split-shell ${activeClaim ? 'drawer-open' : ''}`}>
      <section className={`claim-list-pane ${activeClaim ? 'narrow' : 'full'}`}>
        ...
        <div className="claim-rows-scroll">{items.length ? items.map((r, i) => { const claimId = getClaimId(r); const isOpen = expandedClaim === claimId; const workflowStatus = r.workflowStatus || r.status || r.taskStatus || statusLabel; const claimStatus = r.claimStatus || workflowStatus; return <div key={`${claimId || 'claim'}-${i}`} className={`claim-list-row claim-list-row-wide ${isOpen ? 'active' : ''}`}>{canAssign && <span className="claim-row-check"><input type="checkbox" checked={!!selected[i]} onChange={e => setSelected({ ...selected, [i]: e.target.checked })} /></span>}<span className="claim-expand-dot"><i className={`bi ${isOpen ? 'bi-chevron-down' : 'bi-chevron-right'}`} /></span><span className="claim-list-id"><button className="claim-id-link" type="button" disabled={!claimId} onClick={() => toggleClaim(claimId)}>{claimId || '-'}</button><small>{r.panelName || '-'}</small></span><span className="claim-list-payer" title={r.payerName || ''}>{r.payerName || '-'}</span><span className="claim-list-payer" title={r.patientName || ''}>{r.patientName || '-'}</span><span className="claim-list-payer" title={r.patientId || ''}>{r.patientId || '-'}</span><span className="claim-list-payer" title={r.subscriberId || ''}>{r.subscriberId || '-'}</span><span className="claim-list-payer">{date(r.dateOfService) || '-'}</span><span className="claim-list-amt">{money(r.insuranceBalance)}</span><span className={`badge ${statusClass(claimStatus)}`}>{claimStatus}</span></div>; }) : <div className="claim-empty-panel">No claim records found.</div>}</div>
      </section>
      {activeClaim && <section className="claim-drawer open">
        <div className="claim-drawer-header"><div><div className="claim-drawer-kicker">Claims / {activeTab.label} / Line Level</div><h3>{activeClaimId}</h3><p>{activeClaim.payerName || '-'} - {activeClaim.patientName || '-'} - {money(activeClaim.insuranceBalance)}</p></div><button className="wl-icon" title="Close claim drawer" type="button" onClick={() => { resetDrawerState(); loadClaimTasks(activeClaimId); }}><i className="bi bi-x-lg" /></button></div>
        <div className="claim-drawer-meta"><span><b>Panel</b>{activeClaim.panelName || '-'}</span><span><b>DOB</b>{date(activeClaim.patientDOB)}</span><span><b>DOS</b>{date(activeClaim.dateOfService)}</span><span><b>Clinic</b>{activeClaim.clinicName || '-'}</span><span><b>Provider</b>{activeClaim.referringProvider || '-'}</span><span><b>Assigned</b>{activeClaim.assignedTo || '-'}</span></div>
        <div className="claim-drawer-actions"><button className="wl-icon icon-note" title={readOnlyWorkflow ? 'View claim notes' : 'Claim notes'} type="button" onClick={() => openClaimNotes(activeClaim)}><i className="bi bi-pencil-square" /></button><button className="wl-icon icon-doc" title={readOnlyWorkflow ? 'View documents' : 'Upload documents'} type="button" onClick={() => openClaimDocuments(activeClaimId)}><i className="bi bi-paperclip" /></button>{canAssign && taskView === 'internalEscalation' && <><select className="drawer-reviewer-select" value={bulkReviewer} onChange={e => setBulkReviewer(e.target.value)}><option value="">Select AR Reviewer</option>{arReviewers.map(r => <option key={r.userName || r.UserName} value={r.userName || r.UserName}>{r.displayName || r.DisplayName || r.userName || r.UserName}</option>)}</select><button className="wl-btn teal xs" type="button" disabled={!activeClaimId} onClick={() => assignClaims([activeClaimId], bulkReviewer)}>Assign</button></>}{canAssign && <button className={`wl-btn ${isEscalatedView ? 'amber' : 'red'} xs`} type="button" onClick={() => openEscalation({ mode: isEscalatedView ? 'update' : undefined, level: 'Claim', claimIds: [activeClaimId], title: `${isEscalatedView ? 'Update escalation' : 'Escalate claim'} ${activeClaimId}` })}>{isEscalatedView ? 'Update' : 'Escalate'}</button>}</div>
        <div className="claim-drawer-tabs"><button className={drawerTab === 'lines' ? 'active' : ''} type="button" onClick={() => setDrawerTab('lines')}>Line Tasks</button><button className={drawerTab === 'notes' ? 'active' : ''} type="button" onClick={() => loadClaimDrawerTab('notes', activeClaimId)}>Claim Notes</button><button className={drawerTab === 'documents' ? 'active' : ''} type="button" onClick={() => loadClaimDrawerTab('documents', activeClaimId)}>Documents</button><button className={drawerTab === 'history' ? 'active' : ''} type="button" onClick={() => loadClaimDrawerTab('history', activeClaimId)}>History</button></div>
        <div className="claim-drawer-body">
          {drawerTab === 'lines' && <table className="claim-task-table"><thead><tr><th>Task</th><th>CPT</th><th>Denial</th><th>Classification</th><th>Action</th><th>SLA</th><th>Assigned</th><th className="r">Balance</th></tr></thead><tbody>{activeTasks.length ? activeTasks.map((t, i) => <tr key={t.taskId || i}><td><strong>{t.taskId || '-'}</strong><small>{date(t.createdOn)}</small></td><td><code className="code">{t.cptCode || '-'}</code></td><td><code className="code">{t.denialCode || '-'}</code><small>{t.denialDescription || '-'}</small></td><td>{t.denialClassification || '-'}</td><td>{t.actionCategory || t.recommendedAction || t.task || '-'}</td><td><span className={`badge ${statusClass(t.slaStatus || '')}`}>{t.slaStatus || '-'}</span><small>{t.slaDays ?? '-'} days</small></td><td>{t.assignedTo || '-'}</td><td className="r">{money(t.insuranceBalance)}</td></tr>) : <tr><td colSpan={8} className="empty-cell">Loading or no tasks found for this claim.</td></tr>}</tbody></table>}
    ...
    <div className="lrn-card claim-card">
      ...
      <div className="dt-wrap claim-assign-scroll">
        <table className={`lrn-table workflow-table claim-assign-table thin-bordered ${canAssign ? '' : 'claim-view-only-table'}`}><thead><tr>{canAssign && <th className="sticky-col select-col"><input type="checkbox" checked={all} onChange={e => { const next = {}; if (e.target.checked) items.forEach((_, i) => next[i] = true); setSelected(next); }} /></th>}<th>Claim ID</th><th>Payer Name</th><th>Panel Name</th><th>Patient Name</th><th>Patient DOB</th><th>Date Of Service</th><th>Created On</th><th>Clinic Name</th><th>Referring Provider</th><th className="text-right">Insurance Balance</th><th>Assigned To</th><th>Claim Level Status</th><th>Workflow Status</th>{canAssign && <th>Assign</th>}<th>Actions</th></tr></thead>
          <tbody>{items.length ? items.map((r, i) => { const claimId = getClaimId(r); const isOpen = expandedClaim === claimId; const tasks = claimTasks?.[claimId] || []; const workflowStatus = r.workflowStatus || r.status || r.taskStatus || statusLabel; const claimStatus = r.claimStatus || workflowStatus; return <React.Fragment key={`${claimId || 'claim'}-${i}`}><tr className={`claim-row-ui ${isOpen ? 'open' : ''}`}>{canAssign && <td className="sticky-col select-col"><input type="checkbox" checked={!!selected[i]} onChange={e => setSelected({ ...selected, [i]: e.target.checked })} /></td>}<td><button className="claim-id-link" type="button" disabled={!claimId} onClick={() => toggleClaim(claimId)}><i className={`bi ${isOpen ? 'bi-chevron-down' : 'bi-chevron-right'}`} /> {claimId || '-'}</button></td><td className="payer-full-cell" title={r.payerName || ''}>{r.payerName || '-'}</td><td>{r.panelName || '-'}</td><td>{r.patientName || '-'}</td><td>{date(r.patientDOB)}</td><td>{date(r.dateOfService)}</td><td>{date(r.createdOn)}</td><td>{r.clinicName || '-'}</td><td>{r.referringProvider || '-'}</td><td className="text-right claim-money">{money(r.insuranceBalance)}</td><td className="assigned-to-cell" title={r.assignedTo || ''}>{r.assignedTo || '-'}</td><td><span className={`badge ${statusClass(claimStatus)}`}>{claimStatus}</span></td><td><span className={`badge ${statusClass(workflowStatus)}`}>{workflowStatus}</span></td>{canAssign && <td><button className="wl-btn teal xs" type="button" onClick={() => assignClaims([claimId], bulkReviewer)} disabled={!claimId}>Assign</button></td>}<td className="actions-cell"><div className="claim-row-actions"><button className="wl-icon icon-note" title={readOnlyWorkflow ? 'View claim notes' : 'Claim notes'} type="button" onClick={() => openClaimNotes(r)}><i className="bi bi-pencil-square" /></button><button className="wl-icon icon-doc" title={readOnlyWorkflow ? 'View documents' : 'Upload documents'} type="button" disabled={!claimId} onClick={() => openClaimDocuments(claimId)}><i className="bi bi-paperclip" /></button><button className="wl-icon icon-history" title="Claim history" type="button" disabled={!claimId} onClick={() => openAssignmentHistory(claimId)}><i className="bi bi-clock-history" /></button>{canAssign && (isEscalatedView ? <button className="wl-btn amber xs update-action" type="button" disabled={!claimId} onClick={() => openEscalation({ mode: 'update', level: 'Claim', claimIds: [claimId], title: `Update escalation ${claimId}` })}>Update</button> : <button className="wl-btn red xs" type="button" disabled={!claimId} onClick={() => openEscalation({ level: 'Claim', claimIds: [claimId], title: `Escalate claim ${claimId}` })}>Escalate</button>)}</div></td></tr>{isOpen && <tr className="claim-task-expanded-row"><td colSpan={tableColSpan}><InlineClaimTaskDrill claimId={claimId} tasks={tasks} openClaimNotes={() => openClaimNotes(r)} openLineNotes={openLineNotes} openClaimDocuments={openClaimDocuments} openAssignmentHistory={openAssignmentHistory} readOnlyWorkflow={readOnlyWorkflow} canAssign={canAssign} openEscalation={openEscalation} isEscalatedView={isEscalatedView} /></td></tr>}</React.Fragment>; }) : <tr><td colSpan={tableColSpan} className="empty-cell">No claim records found.</td></tr>}</tbody></table>
