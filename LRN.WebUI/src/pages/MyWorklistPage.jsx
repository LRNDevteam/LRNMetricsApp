...
export default function MyWorklistPage({ labId, user, options, filter, setMessage, onRefreshToken }) {
  ...
  const query = useMemo(() => ({
    ...
    page: 1,
    pageSize: 200
  }), [labId, user, filter, local]);

  async function load() {
    if (!labId) return;
    setLoading(true);
    try {
      const data = await denialWorkflowService.getTasks(query);
      setRows(data?.items || []);
  ...
  return <div className="wl-page">
    ...
    <div className="wl-card wl-filters"><div className="wl-card-hd"><b>My Worklist Filters</b><button className="wl-btn xs" onClick={() => setLocal({ payerName: '', denialClassification: '', actionCategory: '', status: '', searchText: '' })}>Clear all</button></div><div className="wl-filter-grid">
      ...
      <label>Classification<select value={local.denialClassification} onChange={e => setLocal(x => ({ ...x, denialClassification: e.target.value }))}><option value="">All classifications</option>{(options?.denialClassifications || []).map(x => <option key={x}>{x}</option>)}</select></label>
      <label>Action / Task<select value={local.actionCategory} onChange={e => setLocal(x => ({ ...x, actionCategory: e.target.value }))}><option value="">All actions</option>{(options?.actionCategories || []).map(x => <option key={x}>{x}</option>)}</select></label>
      <label>Status<select value={local.status} onChange={e => setLocal(x => ({ ...x, status: e.target.value }))}><option value="">All statuses</option>{statusOptions.map(x => <option key={x}>{x}</option>)}</select></label>
    ...
    <div className="wl-card"><div className="wl-card-hd"><b>AR Reviewer · My Worklist</b><span>{loading ? 'Loading...' : `${claims.length} claim(s)`}</span></div><div className="wl-table-scroll"><table className="wl-main-table"><thead><tr><th></th><th>Claim ID</th><th>Payer</th><th>Panel</th><th>Patient ID</th><th>DOS</th><th>Clinic</th><th>Provider</th><th>Balance</th><th>Claim Notes</th><th>Docs</th><th>Escalate</th><th>Status</th></tr></thead><tbody>{claims.map(c => <React.Fragment key={c.claimId}><tr className={expanded === c.claimId ? 'open' : ''} onClick={() => setExpanded(expanded === c.claimId ? '' : c.claimId)}><td>{expanded === c.claimId ? '▼' : '▶'}</td><td className="linkish">{c.claimId}</td><td>{c.payerName}</td><td>{c.panelName}</td><td>{c.patientId}</td><td>{fmtDate(c.dateOfService)}</td><td>{c.clinicName}</td><td>{c.referringProvider}</td><td className="money">{money(c.balance)}</td><td><button className="wl-icon" onClick={e => { e.stopPropagation(); openNote('Claim', c); }}>📝</button></td><td><button className="wl-icon" onClick={e => { e.stopPropagation(); openDocs(c); }}>📎</button></td><td><button className="wl-btn red xs" onClick={e => { e.stopPropagation(); openEsc('Claim', c); }}>Escalate</button></td><td><span className={`wl-badge ${String(c.status).toLowerCase().replaceAll(' ', '-')}`}>{c.status}</span></td></tr>{expanded === c.claimId && <tr><td colSpan="13" className="wl-detail-cell"><div className="wl-detail-title">CPT / line-level task details</div><div className="wl-line-scroll"><table className="wl-line-table"><thead><tr><th>Task ID</th><th>CPT</th><th>Units</th><th>Modifier</th><th>Denial</th><th>Coverage</th><th>ICD Status</th><th>Classification</th><th>Validity</th><th>Balance</th><th>Action / Task</th><th>SLA</th><th>Status</th><th>Notes</th><th>Escalate</th></tr></thead><tbody>{c.lines.map(l => <tr key={l.taskId}><td><b>{l.taskId}</b></td><td>{l.cptCode}</td><td>{l.units ?? '-'}</td><td>{l.modifier || '-'}</td><td>{l.denialCode}</td><td>{l.coverageStatus || '-'}</td><td>{l.icdComplianceStatus || '-'}</td><td>{l.denialClassification}</td><td>{l.denialValidity || '-'}</td><td className="money">{money(l.insuranceBalance)}</td><td>{l.task || l.recommendedAction}</td><td>{l.daysRemaining ?? '-'}</td><td><span className={`wl-badge ${String(l.status).toLowerCase().replaceAll(' ', '-')}`}>{l.status || 'Open'}</span></td><td><button className="wl-icon" onClick={() => openNote('Line', c, l)}>📝</button></td><td><button className="wl-btn red xs" onClick={() => openEsc('Line', c, l)}>Escalate</button></td></tr>)}</tbody></table></div></td></tr>}</React.Fragment>)}</tbody></table></div></div>
