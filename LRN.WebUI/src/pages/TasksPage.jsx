...
function TaskRow({ task, openNote, openDocs, openEsc, showActionsColumn = true, canEditTask, canEscalateTask, clientManager = false }) {
  ...
  return <tr>
    {showActionsColumn && <td className="sticky-col task-tools-col"><div className="task-tools">{editable ? <><button className="wl-icon" title="Get notes / update comments" onClick={() => openNote(task)}>📝</button><button className="wl-icon" title="Upload document" onClick={() => openDocs(task)}>📎</button>{escalatable && <button className="wl-btn red xs" title="Escalate" onClick={() => openEsc(task)}>Escalate</button>}</> : <span className="muted-text">View only</span>}</div></td>}
