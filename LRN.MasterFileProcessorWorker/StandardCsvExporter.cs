...
public static class StandardCsvExporter
{
		...
		ExportAugmentationContext? augmentation = null)
	{
		...
		while (!parser.EndOfData)
		{
			...
			if (insuranceMaster != null && insuranceMaster.Count > 0)
			{
				...
				{
					var ins = FindInsuranceMasterEntry(insuranceMaster, labId, labName, payerRaw);
		...
		string? payerRaw)
	{
		...
		// The Insurance Master "Lab Name" column stores the insurance reference name, which is
		// not always the same as the configured lab name (e.g. "NorthWest" -> "NWL",
		// "PCRDx - AL" -> "PCR AL"). Translate first, then keep the raw name as a fallback so
		// existing exact matches keep working. See LabInsuranceReferenceMap.
		var insuranceLabName = LabInsuranceReferenceMap.Resolve(labId, labName) ?? labName;

		var candidateLabNames = new List<string>();
		if (!string.IsNullOrWhiteSpace(insuranceLabName))
			candidateLabNames.Add(insuranceLabName!);
		if (!string.IsNullOrWhiteSpace(labName)
			&& !string.Equals(labName, insuranceLabName, StringComparison.OrdinalIgnoreCase))
			candidateLabNames.Add(labName!);

		// 1) Prefer exact lab + payer match, trying the insurance reference name first.
		foreach (var candidate in candidateLabNames)
		{
			var exactKey = BuildInsuranceLookupKey(candidate, payerRaw);
