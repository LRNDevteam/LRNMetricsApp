...
public sealed class ProcessLogWorkbookWriter : IProcessLogWorkbookWriter
{
    ...
    private readonly ILogger<ProcessLogWorkbookWriter> _logger;

    private string? _cachedDriveId;

    public ProcessLogWorkbookWriter(IOptions<ProcessLogOptions> logOpt, IOptions<ImportOptions> importOpt, SharePointDownloader sp, ILogger<ProcessLogWorkbookWriter> logger)
    {
    ...
    public Task UpsertRunAsync(DateTime nowIst, RunLogRow row, CancellationToken ct)
    {
        ...
        if (!TryWrite(() => ProcessLogWorkbook.UpsertRun(folder, _logOpt.WorkbookFilePrefix, nowIst, row), nameof(UpsertRunAsync), row.RunID))
            return Task.CompletedTask;
    ...
    public Task UpsertStepAsync(DateTime nowIst, StepLogRow row, CancellationToken ct)
    {
        ...
        var folder = ResolveFolder(nowIst);
        if (!TryWrite(() => ProcessLogWorkbook.UpsertStep(folder, _logOpt.WorkbookFilePrefix, nowIst, row), nameof(UpsertStepAsync), row.RunID))
            return Task.CompletedTask;
    ...
    public Task AppendErrorAsync(DateTime nowIst, ErrorLogRow row, CancellationToken ct)
    {
        ...
        var folder = ResolveFolder(nowIst);
        if (!TryWrite(() => ProcessLogWorkbook.AppendError(folder, _logOpt.WorkbookFilePrefix, nowIst, row), nameof(AppendErrorAsync), row.RunID))
            return Task.CompletedTask;
    ...
    public Task IncrementRunCountAsync(DateTime nowIst, string runId, bool isWarning, CancellationToken ct)
    {
        ...
        var folder = ResolveFolder(nowIst);
        if (!TryWrite(() => ProcessLogWorkbook.IncrementRunCount(folder, _logOpt.WorkbookFilePrefix, nowIst, runId, isWarning), nameof(IncrementRunCountAsync), runId))
            return Task.CompletedTask;
    ...
    /// <summary>
    /// Runs a workbook write and swallows file-lock/IO failures (e.g. the .xlsx is open
    /// in Excel or locked by SharePoint sync) so a busy log file never aborts the pipeline.
    /// </summary>
    private bool TryWrite(Action write, string operation, string runId)
    {
        try
        {
            write();
        ...
        {
            _logger.LogWarning(ex, "Skipping process-log workbook write ({Operation}, RunID={RunId}) because the file is locked/inaccessible.", operation, runId);
            return false;
        }
    }
