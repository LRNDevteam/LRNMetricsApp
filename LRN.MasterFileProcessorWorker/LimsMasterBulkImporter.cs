...
public sealed class LimsMasterBulkImporter
{
		...
		CancellationToken ct)
	{
		...
		while (reader.Read())
		{
			...
			foreach (var loadColumn in columnsToLoad)
			{
				...
				{
					// For mixed text/date columns: convert OA date serial numbers to readable date strings.
					if (loadColumn.ParseAsDate && !string.IsNullOrWhiteSpace(text))
						text = TryConvertOaDateSerial(text) ?? text;
		...
		LimsImportRequest request)
	{
		...
		foreach (var dest in destinationColumns.Where(c => !c.IsIdentity && !c.IsComputed))
		{
			...
			{
				loadColumns.Add(new LimsLoadColumn(dest, matchedHeader.ColumnIndex, LimsSpecialColumn.None, mapping?.ParseAsDate == true));
			}
		}
	...
	/// <summary>
	/// Converts a raw numeric OA date serial string (e.g. "45876") to "MM/dd/yyyy".
	/// Returns null if the value is not a valid OA date serial (text like "Canceled" is left unchanged by the caller).
	/// </summary>
	private static string? TryConvertOaDateSerial(string? value)
	{
		if (string.IsNullOrWhiteSpace(value)) return null;
		if (!double.TryParse(value.Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out var oaDate))
			return null;
		// Valid OA date range: 1 (Jan 1 1900) to ~109574 (Dec 31 2199).
		// Values outside this range are treated as plain numbers, not dates.
		if (oaDate < 1 || oaDate > 109574)
			return null;
...
ORDER BY c.column_id;";
	...
	private sealed record LimsLoadColumn(SqlDestinationColumn DestinationColumn, int? ExcelColumnIndex, LimsSpecialColumn SpecialColumn, bool ParseAsDate = false);
	...
	private sealed class LimsSchemaMapping
	{
		...
		public static List<LimsColumnMapping> Load(string? schemaPath)
		{
			...
			foreach (var col in columns.EnumerateArray())
			{
				...
				if (col.TryGetProperty("ParseAsDate", out var pad) && pad.ValueKind == JsonValueKind.True)
					mapping.ParseAsDate = true;
...
public sealed class LimsColumnMapping
{
	...
	// When true, a raw numeric value in this column is treated as an Excel OA date serial
	// and converted to "MM/dd/yyyy". Use for nvarchar columns that hold mixed text/date values.
	public bool ParseAsDate { get; set; }
}
