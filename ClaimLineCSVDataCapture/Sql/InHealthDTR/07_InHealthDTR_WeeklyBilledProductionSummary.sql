-- InHealthDTR — Weekly Claim Production Billed Summary
-- Rule:
...
--   Note    : InHealthDTR week runs Monday through Sunday (default).
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'InH_WeeklyBilledProductionSummary')
CREATE TABLE dbo.InH_WeeklyBilledProductionSummary
(
...
CREATE OR ALTER PROCEDURE dbo.usp_RefreshInH_WeeklyBilledProductionSummary
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Anchor to most recent Monday using ChargeEnteredDate from data
    SELECT
    ...
    TRUNCATE TABLE dbo.InH_WeeklyBilledProductionSummary;

    INSERT INTO dbo.InH_WeeklyBilledProductionSummary
    ...
    PRINT 'usp_RefreshInH_WeeklyBilledProductionSummary completed — ' + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' rows.';
END
GO

PRINT '07_InHealthDTR_WeeklyBilledProductionSummary.sql completed.';
