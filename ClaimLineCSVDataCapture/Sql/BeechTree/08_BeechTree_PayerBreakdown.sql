-- BeechTree — Payer Breakdown  +  Payer × Panel
--
-- Filter (both SPs):
--   TRY_CAST(FirstBilledDate AS DATE) IS NOT NULL
--   AND NULLIF(LTRIM(RTRIM(PayerName_Raw)), '') IS NOT NULL
--
-- Table 1 – BT_PayerBreakdown   (Payer × ChargeEnteredDate Month)
-- Table 2 – BT_PayerByPanel     (Payer × Panelname)
-- ============================================================

SET NOCOUNT ON;
GO

IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'BT_PayerBreakdown')
CREATE TABLE dbo.BT_PayerBreakdown
(
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'BT_PayerByPanel')
CREATE TABLE dbo.BT_PayerByPanel
(
    SummaryId    INT             NOT NULL IDENTITY(1,1) PRIMARY KEY,
    PayerName    NVARCHAR(500)   NOT NULL,
...
BEGIN
    ...
    TRUNCATE TABLE dbo.BT_PayerBreakdown;

    INSERT INTO dbo.BT_PayerBreakdown (PayerName, BilledYearMonth, ClaimCount, TotalCharges, RefreshedAt)
    ...
    PRINT 'usp_RefreshBT_PayerBreakdown completed — ' + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' rows.';
END
GO

CREATE OR ALTER PROCEDURE dbo.usp_RefreshBT_PayerByPanel
AS
BEGIN
    SET NOCOUNT ON;
    ...
    TRUNCATE TABLE dbo.BT_PayerByPanel;

    INSERT INTO dbo.BT_PayerByPanel (PayerName, PanelType, ClaimCount, TotalCharges, RefreshedAt)
