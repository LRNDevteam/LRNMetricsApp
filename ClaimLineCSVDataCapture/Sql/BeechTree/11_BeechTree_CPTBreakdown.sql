-- BeechTree — CPT Breakdown (line-level)
-- Rule:
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'BT_CPTBreakdown')
CREATE TABLE dbo.BT_CPTBreakdown
(
...
CREATE OR ALTER PROCEDURE dbo.usp_RefreshBT_CPTBreakdown
AS
BEGIN
    SET NOCOUNT ON;
    ...
    TRUNCATE TABLE dbo.BT_CPTBreakdown;

    INSERT INTO dbo.BT_CPTBreakdown
    ...
    PRINT 'usp_RefreshBT_CPTBreakdown completed — ' + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' rows.';
END
GO

PRINT '11_BeechTree_CPTBreakdown.sql completed.';
