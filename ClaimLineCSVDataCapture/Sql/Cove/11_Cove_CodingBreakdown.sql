-- COVE Labs — Coding (Unbilled) Breakdown
-- Rule:
...
--
-- Storage:
--   Cove_CodingPanelSummary  — one row per Panelname
--   Cove_CodingCPTDetail     — one row per Panelname + CPTCodeXUnitsXModifier
-- ============================================================
...
-- ============================================================
-- Step 0: Ensure CPTCodeXUnitsXModifier column exists in ClaimLevelData
-- ============================================================
IF NOT EXISTS (
...
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Cove_CodingPanelSummary')
CREATE TABLE dbo.Cove_CodingPanelSummary
(
...
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Cove_CodingCPTDetail')
CREATE TABLE dbo.Cove_CodingCPTDetail
(
...
-- ============================================================
CREATE OR ALTER PROCEDURE dbo.usp_RefreshCove_CodingBreakdown_Unbilled
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Atomic replace
    TRUNCATE TABLE dbo.Cove_CodingPanelSummary;
    INSERT INTO dbo.Cove_CodingPanelSummary (PanelName, ClaimCount, TotalCharges, RefreshedAt)
    SELECT Panelname, ClaimCount, TotalCharges, GETDATE()
    FROM #PanelSummary
    ORDER BY TotalCharges DESC;

    TRUNCATE TABLE dbo.Cove_CodingCPTDetail;
    INSERT INTO dbo.Cove_CodingCPTDetail
    ...
    PRINT 'usp_RefreshCove_CodingBreakdown_Unbilled completed.';
END
GO

/*
SELECT PanelName, ClaimCount, TotalCharges FROM dbo.Cove_CodingPanelSummary ORDER BY TotalCharges DESC;
SELECT PanelName, CPTCodeXUnitsXModifier, ClaimCount, TotalCharges FROM dbo.Cove_CodingCPTDetail ORDER BY PanelName, TotalCharges DESC;
*/

PRINT '11_Cove_CodingBreakdown.sql completed.';
