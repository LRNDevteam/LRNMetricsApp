...
BEGIN
    ...
    SELECT ESYear, ESMonth, v.RowCode, v.Description, ISNULL(v.MetricValue, 0)
    FROM (
        -- Per-month
    ...
    -- Separate INSERTs (no UNION ALL) so pt is always bound for every filter
    -- that triggers the live LIS scan (panel/clinic/provider/rep/FirstBilledDate).
    DROP TABLE IF EXISTS #CpExceptionPanels;
    SELECT DISTINCT l.PanelType
    INTO #CpExceptionPanels
    FROM #Lis AS l
    WHERE l.NewStatus = N'Billable'
      AND l.BillCategory IN (N'Not Billed', N'Unbilled')
      AND l.SubStatus = N'CP Exception'
      AND NULLIF(l.PanelType, N'') IS NOT NULL;
    ...
    SELECT p.LISYear, p.LISMonth,
           ...
           CAST(COUNT(DISTINCT CASE
               WHEN l.SubStatus = N'CP Exception' AND l.PanelType = cp.PanelType
               THEN l.Accession END) AS DECIMAL(18,2))
    FROM (SELECT DISTINCT LISYear, LISMonth FROM #Lis) AS p
    CROSS JOIN #CpExceptionPanels AS cp
    LEFT JOIN #Lis AS l
           ON l.LISYear = p.LISYear AND l.LISMonth = p.LISMonth
          AND l.NewStatus = N'Billable'
          AND l.BillCategory IN (N'Not Billed', N'Unbilled')
    GROUP BY p.LISYear, p.LISMonth, cp.PanelType;

    INSERT INTO #LisRows (ESYear, ESMonth, RowCode, Description, MetricValue)
    SELECT 0, 0,
           N'D.6.' + cp.PanelType,
           N'    ' + cp.PanelType,
           CAST(COUNT(DISTINCT l.Accession) AS DECIMAL(18,2))
    FROM #CpExceptionPanels AS cp
    INNER JOIN #Lis AS l
            ON l.PanelType = cp.PanelType
           AND l.NewStatus = N'Billable'
           AND l.BillCategory IN (N'Not Billed', N'Unbilled')
           AND l.SubStatus = N'CP Exception'
    GROUP BY cp.PanelType;

    -- ── D.5.{PanelType} Coding exception by panel (same pattern, any live filter)
    DROP TABLE IF EXISTS #CodingExceptionPanels;
    SELECT DISTINCT l.PanelType
    INTO #CodingExceptionPanels
    FROM #Lis AS l
    WHERE l.NewStatus = N'Billable'
      AND l.BillCategory IN (N'Not Billed', N'Unbilled')
      AND l.SubStatus = N'Coding exception'
      AND NULLIF(l.PanelType, N'') IS NOT NULL;
    ...
    SELECT p.LISYear, p.LISMonth,
           ...
           N'    ' + ce.PanelType,
           CAST(COUNT(DISTINCT CASE
               WHEN l.SubStatus = N'Coding exception' AND l.PanelType = ce.PanelType
               THEN l.Accession END) AS DECIMAL(18,2))
    FROM (SELECT DISTINCT LISYear, LISMonth FROM #Lis) AS p
    CROSS JOIN #CodingExceptionPanels AS ce
    LEFT JOIN #Lis AS l
           ON l.LISYear = p.LISYear AND l.LISMonth = p.LISMonth
          AND l.NewStatus = N'Billable'
          AND l.BillCategory IN (N'Not Billed', N'Unbilled')
    GROUP BY p.LISYear, p.LISMonth, ce.PanelType;
    ...
    SELECT 0, 0,
           ...
           N'    ' + ce.PanelType,
           CAST(COUNT(DISTINCT l.Accession) AS DECIMAL(18,2))
    FROM #CodingExceptionPanels AS ce
    INNER JOIN #Lis AS l
            ON l.PanelType = ce.PanelType
           AND l.NewStatus = N'Billable'
           AND l.BillCategory IN (N'Not Billed', N'Unbilled')
           AND l.SubStatus = N'Coding exception'
    GROUP BY ce.PanelType;
    ...
    -- ════════════════════════════════════════════════════════════════════
    INSERT INTO #LisRows (ESYear, ESMonth, RowCode, Description, MetricValue)
    SELECT agg.ESYear, agg.ESMonth, v.RowCode, v.Description, ISNULL(v.MetricValue, 0)
    FROM (
        -- Per-month breakdown
    ...
    CROSS APPLY (VALUES
        ...
        ('O',   'Total Billed ($)',                                   CAST(ISNULL(agg.amtO,  0) AS DECIMAL(18,2))),
        ('P',   'Insurance Payment ($)',                              CAST(ISNULL(agg.amtP,  0) AS DECIMAL(18,2))),
        ('Q',   'Patient Responsibility ($)',                         CAST(ISNULL(agg.amtQ,  0) AS DECIMAL(18,2))),
        ('R',   'Patient Payment ($)',                                CAST(ISNULL(agg.amtR,  0) AS DECIMAL(18,2))),
        ('S',   'Adjustments / Write Off ($)',                        CAST(ISNULL(agg.amtS,  0) AS DECIMAL(18,2))),
        ('T',   'Partially Paid ($)',                                 CAST(ISNULL(agg.amtT,  0) AS DECIMAL(18,2))),
        ('U',   'Insurance Balance ($)',                              CAST(ISNULL(agg.amtU,  0) AS DECIMAL(18,2))),
        ('U.1', '  Denials',                                          CAST(ISNULL(agg.amtU1, 0) AS DECIMAL(18,2))),
        ('U.2', '  Partially Denied',                                 CAST(ISNULL(agg.amtU2, 0) AS DECIMAL(18,2))),
        ('U.3', '  No Response from Payor',                           CAST(ISNULL(agg.amtU3, 0) AS DECIMAL(18,2))),
        ('V',   'Average Payment ($) - Total Pay/Billed Claims',      CAST(CASE WHEN ISNULL(agg.cntV, 0) > 0 THEN ISNULL(agg.sumV, 0) / agg.cntV ELSE 0 END AS DECIMAL(18,2))),
        ('W',   'Average Payment ($) - Total Pay/Paid Claims',        CAST(CASE WHEN ISNULL(agg.cntW, 0) > 0 THEN ISNULL(agg.sumW, 0) / agg.cntW ELSE 0 END AS DECIMAL(18,2))),
        ('X',   'Average Payment ($) - Total Pay/Adjudicated Claims', CAST(CASE WHEN ISNULL(agg.cntX, 0) > 0 THEN ISNULL(agg.sumX, 0) / agg.cntX ELSE 0 END AS DECIMAL(18,2)))
    ) v(RowCode, Description, MetricValue);
    ...
    BEGIN
        ...
        SELECT b.ESYear, b.ESMonth, 'G', '  Billed Mismatches - Accessions NA / Other Sample',
               CAST(CASE WHEN ISNULL(b.BilledBase, 0) - ISNULL(l.BilledLis, 0) > 0
                         THEN ISNULL(b.BilledBase, 0) - ISNULL(l.BilledLis, 0) ELSE 0 END AS DECIMAL(18,2))
        FROM   #GBase b
        ...
        SELECT 0, 0, 'G', '  Billed Mismatches - Accessions NA / Other Sample',
               CAST(CASE WHEN ISNULL(tb.TotBase, 0) - ISNULL(tl.TotLis, 0) > 0
                         THEN ISNULL(tb.TotBase, 0) - ISNULL(tl.TotLis, 0) ELSE 0 END AS DECIMAL(18,2))
