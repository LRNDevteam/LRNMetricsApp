-- ============================================================
-- Cove Collection — Insurance Vs Payments (payer-flat, correct logic)
-- Date: 2026-09-16
-- Run on CoveLRN. Safe to re-run.
--
-- Client report layout:
--   Filter : InsurancePayment > 0  ONLY (no CheckDate requirement)
--   Rows   : PayerName_Raw
--   Values : Count(ClaimID) — NOT DISTINCT (matches Excel Count of VisitNum)
--            Sum(InsurancePayment)
--   No CheckDate / month columns (BillYear=0, BillMonth=0 in snapshot)
-- ============================================================
SET NOCOUNT ON;
GO

PRINT 'Creating usp_RefreshCove_CS_InsuranceVsPayment (payer-flat)...';
GO
...
BEGIN
    ...
    PRINT 'usp_RefreshCove_CS_InsuranceVsPayment completed — '
        + CAST(@@ROWCOUNT AS VARCHAR(20)) + ' payer rows.';
END
GO

PRINT 'Creating usp_GetCove_CS_InsuranceVsPayment (payer-flat)...';
GO
...
PRINT 'Refreshing Cove_CS_InsuranceVsPayment (payer-flat)...';
EXEC dbo.usp_RefreshCove_CS_InsuranceVsPayment;
GO

SELECT COUNT(*) AS PayerRows,
       SUM(NoOfPaidClaims) AS TotalClaims,
       SUM(InsurancePayment) AS TotalPay
FROM dbo.Cove_CS_InsuranceVsPayment;

SELECT PayerName, NoOfPaidClaims, InsurancePayment
FROM dbo.Cove_CS_InsuranceVsPayment
WHERE PayerName IN ('MEDICARE FL', 'MEDICARE TX')
ORDER BY PayerName;

PRINT 'FIX_Cove_CS_InsuranceVsPayment_PayerFlat complete.';
GO
