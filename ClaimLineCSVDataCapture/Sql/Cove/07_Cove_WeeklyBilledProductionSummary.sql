-- COVE Labs — Weekly Claim Production Billed Summary
-- Rule:
--   Filter  : TRY_CAST(FirstBilledDate AS DATE) IS NOT NULL
--   Rows    : Panelname  x  Top 3 Payer (by COUNT(DISTINCT ClaimID), per Panelname)
--   Columns : FirstBilledDate week range Fri–Thu, last 4 complete weeks
--             | COUNT(DISTINCT ClaimID) | SUM(ChargeAmount)
--   Note    : COVE week runs Friday through Thursday (not Mon–Sun).
...
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Cove_WeeklyBilledProductionSummary')
CREATE TABLE dbo.Cove_WeeklyBilledProductionSummary
(
...
BEGIN
    ...
    INSERT INTO dbo.Cove_WeeklyBilledProductionSummary
    ...
    PRINT 'usp_RefreshCove_WeeklyBilledProductionSummary completed — ' + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' rows.';
END
GO
...
PRINT '07_Cove_WeeklyBilledProductionSummary.sql completed.';
