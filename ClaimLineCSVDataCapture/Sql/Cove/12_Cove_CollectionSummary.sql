...
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Cove_CS_PanelAverages') AND name = 'ClaimCount')
    ALTER TABLE dbo.Cove_CS_PanelAverages ADD ClaimCount INT NOT NULL DEFAULT 0;
GO
...
-- 9. Rep vs Payment
-- SalesRepname is often NULL on paid claims. The target column is NOT NULL, so
-- coalesce to 'Unknown' (same pattern as Panel vs Payment). Also require a real
-- CheckDate so CheckYear / CheckMonth cannot be NULL.
...
-- 11b. CPT vs Payment %  (was EXECed but never created — snapshot PaymentPct stayed 0)
