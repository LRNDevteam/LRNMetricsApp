-- Certus Labs — CPT Breakdown (line-level)
-- Rule:
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Cert_CPTBreakdown')
CREATE TABLE dbo.Cert_CPTBreakdown
(
...
CREATE OR ALTER PROCEDURE dbo.usp_RefreshCert_CPTBreakdown
AS
BEGIN
    SET NOCOUNT ON;
    ...
    TRUNCATE TABLE dbo.Cert_CPTBreakdown;

    INSERT INTO dbo.Cert_CPTBreakdown
    ...
    PRINT 'usp_RefreshCert_CPTBreakdown completed — ' + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' rows.';
END
GO
...
PRINT '10_Certus_CPTBreakdown.sql completed.';
