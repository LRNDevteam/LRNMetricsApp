-- =====================================================================
-- RisingTides - Collection Summary READ Stored Procedures
...
--   1) No filters  -> read pre-aggregated dbo.RT_CS_* snapshot tables.
...
-- Source: dbo.LineLevelData / dbo.RT_CS_MonthlyClaimVolume
...
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_MonthlyClaimVolume
...
BEGIN
    ...
    BEGIN
        ...
        FROM    dbo.RT_CS_MonthlyClaimVolume
...
BEGIN
    ...
    BEGIN
        SELECT  PanelName,
                ...
                InsurancePayment,
                CAST(InsurancePayment / NULLIF(NoOfClaims, 0) AS DECIMAL(18,2)) AS AveragePaidAmount
        FROM    dbo.RT_CS_WeeklyClaimVolume
...
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_Top5ReimbursementPct
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.RT_CS_Top5ReimbursementPct
        ORDER  BY PayerRank;
        RETURN;
    END;
...
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_Top5ReimbursementPay
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.RT_CS_Top5ReimbursementPay
        ORDER  BY PayerRank;
        RETURN;
    END;
...
-- =====================================================================
-- 5. Panel Averages
-- Source: dbo.ClaimLevelData / dbo.RT_CS_PanelAverages
-- No-filter: reads snapshot.
-- Filter: live aggregate from ClaimLevelData using CheckDate.
-- Note: RT_CS_PanelAverages has no AdjudicatedCount/Amount columns;
--       0 placeholders are returned to satisfy the shared C# reader.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_PanelAverages
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
-- Source: dbo.ClaimLevelData / dbo.RT_CS_AvgPayments
-- Aliases: ClaimCount->NoOfClaims, InsurancePayment->CarrierPayment,
--          Over30/60->Days30/60 (to match C# reader).
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_AvgPayments
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
-- =====================================================================
-- 8. Panel vs Payment
-- Source: dbo.ClaimLevelData / dbo.RT_CS_PanelVsPayment
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_PanelVsPayment
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
-- =====================================================================
-- 9. Rep vs Payment
-- Source: dbo.ClaimLevelData / dbo.RT_CS_RepVsPayment
-- Uses CheckDate (RisingTides date field) instead of CheckDate.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_RepVsPayment
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
-- Source: dbo.ClaimLevelData / dbo.RT_CS_InsuranceVsPaymentPct
-- No-filter: returns PanelGroupCount AS NoOfClaims (snapshot column).
-- Filter: live re-aggregate using AVG(PaymentPercent) * 100.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_InsuranceVsPaymentPct
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        SELECT  PayerName,
                ...
                InsurancePayment,
                PaymentPct
        FROM    dbo.RT_CS_InsuranceVsPaymentPct
...
-- =====================================================================
-- 11. CPT vs Payment %
-- Source: dbo.LineLevelData / dbo.RT_CS_CptVsPaymentPct
-- =====================================================================
...
-- =====================================================================
-- 12. Status Summary
-- Source: dbo.ClaimLevelData / dbo.RT_CS_StatusSummary
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_StatusSummary
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
-- =====================================================================
-- 13. Provider Summary
-- Source: dbo.ClaimLevelData / dbo.RT_CS_ProviderSummary
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_ProviderSummary
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.RT_CS_ProviderSummary
...
-- Source: dbo.ClaimLevelData / dbo.RT_CS_InsuranceVsPayment (no snapshot)
-- Uses CheckDate (RisingTides date field) instead of CheckDate.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetRT_CS_InsuranceVsPayment
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
