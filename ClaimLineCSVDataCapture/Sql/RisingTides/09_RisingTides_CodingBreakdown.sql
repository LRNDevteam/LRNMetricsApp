-- RisingTides — Coding (Billed) Breakdown
-- Rule:
...
--
-- Storage:
--   RT_CodingPanelSummary  — one row per Panelname
--   RT_CodingCPTDetail     — one row per Panelname + CPTCodeXUnitsXModifier
-- ============================================================
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'RT_CodingPanelSummary')
CREATE TABLE dbo.RT_CodingPanelSummary
(
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'RT_CodingCPTDetail')
CREATE TABLE dbo.RT_CodingCPTDetail
(
...
CREATE OR ALTER PROCEDURE dbo.usp_RefreshRT_CodingBreakdown_Billed
AS
BEGIN
    SET NOCOUNT ON;
    ...
    INSERT INTO dbo.RT_CodingPanelSummary (PanelName, ClaimCount, TotalCharges, RefreshedAt)
    SELECT Panelname, ClaimCount, TotalCharges, GETDATE() FROM #PanelSummary ORDER BY TotalCharges DESC;

    TRUNCATE TABLE dbo.RT_CodingCPTDetail;
    INSERT INTO dbo.RT_CodingCPTDetail (PanelName, CPTCodeXUnitsXModifier, ClaimCount, TotalCharges, RefreshedAt)
    ...
    PRINT 'usp_RefreshRT_CodingBreakdown_Billed completed.';
END
GO

PRINT '09_RisingTides_CodingBreakdown.sql completed.';
