-- PhiLife — Monthly Claim Production Billed Summary
-- Rule:
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Phi_MonthlyBilledProductionSummary')
CREATE TABLE dbo.Phi_MonthlyBilledProductionSummary
(
...
CREATE OR ALTER PROCEDURE dbo.usp_RefreshPhi_MonthlyBilledProductionSummary
AS
BEGIN
    SET NOCOUNT ON;
    ...
    TRUNCATE TABLE dbo.Phi_MonthlyBilledProductionSummary;

    INSERT INTO dbo.Phi_MonthlyBilledProductionSummary
    ...
    PRINT 'usp_RefreshPhi_MonthlyBilledProductionSummary completed — '
          + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' rows.';
END
GO

PRINT '06_PhiLife_MonthlyBilledProductionSummary.sql completed.';
