...
BEGIN
    ...
    SELECT
        ...
        FORMAT(TRY_CAST(CheckDate AS DATE), 'yyyy-MM')  AS BilledYearMonth,
        COUNT(*)                                                AS ClaimCount,
        ISNULL(SUM(TRY_CAST(InsurancePayment AS DECIMAL(18,2))),0)  AS TotalCharges
    INTO #BilledRaw
    FROM dbo.ClaimLevelData
    WHERE TRY_CAST(InsurancePayment AS DECIMAL(18,2)) > 0 

	
	--LTRIM(RTRIM(ClaimStatus)) NOT IN (
 --             'Unbilled in Daq',
 --             'Unbilled in Daq - PR',
 --             'Unbilled in Webpm',
 ...
 --         )
    ...
    GROUP BY
        ...
        FORMAT(TRY_CAST(CheckDate AS DATE), 'yyyy-MM');
