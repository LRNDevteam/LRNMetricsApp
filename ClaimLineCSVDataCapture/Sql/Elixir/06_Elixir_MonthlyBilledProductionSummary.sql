-- Elixir Labs — Monthly Claim Production Billed Summary
-- Rule:
...
-- ============================================================
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Elix_MonthlyBilledProductionSummary')
CREATE TABLE dbo.Elix_MonthlyBilledProductionSummary
(
...
-- ============================================================
CREATE OR ALTER PROCEDURE dbo.usp_RefreshElix_MonthlyBilledProductionSummary
AS
BEGIN
    SET NOCOUNT ON;

    -- Aggregate by Panelname x PayerName_Raw x FirstBilledDate month.
    -- No payer exclusions for Elixir.
    SELECT
    ...
    TRUNCATE TABLE dbo.Elix_MonthlyBilledProductionSummary;

    INSERT INTO dbo.Elix_MonthlyBilledProductionSummary
    ...
    PRINT 'usp_RefreshElix_MonthlyBilledProductionSummary completed — ' + CAST(@@ROWCOUNT AS NVARCHAR(20)) + ' rows.';
END
GO
...
PRINT '06_Elixir_MonthlyBilledProductionSummary.sql completed.';
