...
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_Top5ReimbursementPct
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.Elix_CS_Top5ReimbursementPct
        ORDER  BY PayerRank;
        RETURN;
    END;
...
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_Top5ReimbursementPay
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.Elix_CS_Top5ReimbursementPay
        ORDER  BY PayerRank;
        RETURN;
    END;
...
-- =====================================================================
-- Elixir - Collection Summary READ Stored Procedures
...
--   1) No filters  -> read pre-aggregated dbo.Elix_CS_* snapshot tables.
...
-- Source: dbo.LineLevelData / dbo.Elix_CS_MonthlyClaimVolume
...
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_MonthlyClaimVolume
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM    dbo.Elix_CS_MonthlyClaimVolume
...
-- Source: dbo.LineLevelData / dbo.Elix_CS_WeeklyClaimVolume
-- Filter: InsurancePayment > 0, valid PostingDate
-- No-filter path reads aggregate values.
-- Filter path calculates last 4 weeks from latest PostingDate in filtered data.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_WeeklyClaimVolume
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        SELECT  PanelName,
                ...
                InsurancePayment,
                CAST(InsurancePayment / NULLIF(NoOfClaims, 0) AS DECIMAL(18,2)) AS AveragePaidAmount
        FROM    dbo.Elix_CS_WeeklyClaimVolume
...
-- =====================================================================
-- 5. Panel Averages
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_PanelAverages
-- No-filter: reads snapshot.
-- Filter: live aggregate from ClaimLevelData using PostingDate.
-- Note: Elix_CS_PanelAverages has no AdjudicatedCount/Amount columns;
--       0 placeholders are returned to satisfy the shared C# reader.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_PanelAverages
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM    dbo.Elix_CS_PanelAverages
...
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_AvgPayments
-- Aliases: ClaimCount->NoOfClaims, InsurancePayment->CarrierPayment,
--          Over30/60->Days30/60 (to match C# reader).
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_AvgPayments
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    ;WITH base AS (
        ...
        WHERE ISNULL(TRY_CAST(InsurancePayment AS DECIMAL(18,2)), 0) > 0
          AND TRY_CAST(PostingDate AS DATE) IS NOT NULL
          AND TRY_CAST(PostingDate AS DATE) >= @Cutoff
...
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_InsuranceVsAging
-- AgingBucket values: Current / 30 Days / 60 Days / 90 Days / 120+ Days
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_InsuranceVsAging
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.Elix_CS_InsuranceVsAging
...
-- =====================================================================
-- 8. Panel vs Payment
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_PanelVsPayment
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_PanelVsPayment
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM    dbo.Elix_CS_PanelVsPayment
...
-- =====================================================================
-- 9. Rep vs Payment
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_RepVsPayment
-- Uses PostingDate (Elixir date field) instead of CheckDate.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_RepVsPayment
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    SELECT
        ...
        CAST(YEAR (TRY_CAST(PostingDate AS DATE)) AS INT)                   AS CheckYear,
        CAST(MONTH(TRY_CAST(PostingDate AS DATE)) AS INT)                   AS CheckMonth,
    ...
    GROUP BY
        ...
        CAST(MONTH(TRY_CAST(PostingDate AS DATE)) AS INT)
    ORDER BY SalesRepName, CheckYear, CheckMonth;
END
GO
...
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_InsuranceVsPaymentPct
-- No-filter: returns PanelGroupCount AS NoOfClaims (snapshot column).
-- Filter: live re-aggregate using AVG(PaymentPercent) * 100.
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_InsuranceVsPaymentPct
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        SELECT  PayerName,
                ...
                InsurancePayment,
                PaymentPct
        FROM    dbo.Elix_CS_InsuranceVsPaymentPct
...
-- =====================================================================
-- 11. CPT vs Payment %
-- Source: dbo.LineLevelData / dbo.Elix_CS_CptVsPaymentPct
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_CptVsPaymentPct
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.Elix_CS_CptVsPaymentPct
...
-- =====================================================================
-- 12. Status Summary
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_StatusSummary
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_StatusSummary
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.Elix_CS_StatusSummary;
        RETURN;
    END;
...
-- =====================================================================
-- 13. Provider Summary
-- Source: dbo.ClaimLevelData / dbo.Elix_CS_ProviderSummary
-- =====================================================================
CREATE OR ALTER PROCEDURE dbo.usp_GetElix_CS_ProviderSummary
    @PayerNames      NVARCHAR(MAX) = NULL,
    @PanelNames      NVARCHAR(MAX) = NULL,
    @DosFrom         DATE          = NULL,
    @DosTo           DATE          = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        FROM   dbo.Elix_CS_ProviderSummary
