-- ============================================================
-- PCRLabsofAmerica – Executive Summary Detail (Drill-Down) SP
-- File : 18_PCRLOA_ExecutiveSummary_Detail.sql
--
-- Returns the individual rows that make up a single cell in the
-- Executive Summary grid.
--
-- Parameters
--   @Category  – 'PMS' | 'Cash' | 'LIS'
--   @RowCode   – e.g. 'O','W1','AD' (PMS/Cash, from ClaimLevelData)
--                 or 'A'..'I5' (LIS, from dbo.LIMSMaster)
--   @Year      – calendar year  (0 = all years)
--   @Month     – calendar month (0 = all months within the year)
--
-- LIS rows are sourced from dbo.LIMSMaster using the same RoleID
-- filter logic as 16_PCRLOA_ExecutiveSummary_Aggregate.sql's #Lis
-- population. Display columns (PatientName, PayerName, PanelName,
-- ClinicName, BillingProvider) are auto-detected from candidate
-- column names since LIMSMaster's confirmed columns are limited to
-- RequestCollectDate/InsuranceCategory/BilledorNot/RessultedStatus/
-- ClaimStatus/ClientStatus/Accession/SourceFile/CreatedOn (OrderStatus
-- is auto-detected too). LIS has no dollar amounts, so ChargeAmount/
-- InsurancePayment/PatientPayment/InsuranceBalance/PatientBalance are
-- returned as 0.
-- ============================================================
SET NOCOUNT ON;
GO
...
PRINT '18_PCRLOA_ExecutiveSummary_Detail.sql completed.';
GO
