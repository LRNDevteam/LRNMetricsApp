...
public interface IDenialDatabaseRerunRepository
{
    Task<DenialDatabaseRerunSubmitResult> RequestAsync(
        IReadOnlyList<DenialDatabaseRerunLabSelection> labs,
        DenialDatabaseRerunContext context,
        CancellationToken ct);

    Task<IReadOnlyList<DenialDatabaseRerunRow>> GetRecentAsync(int take, CancellationToken ct);
}

/// <summary>
/// Queues Denial Database Service re-runs into <c>LRNMaster.dbo.DenialDatabaseRerunRequest</c> - the
/// Denial Database Service equivalent of <see cref="SqlMasterProcessorRerunRepository"/>.
///
/// <para>
/// The dashboard cannot call LRN.DenialDatabaseWorker directly: it is a Windows Service polling on a
/// timer in a different process. So a re-run is a row. The worker claims it on its next poll, runs
/// the lab with its already-processed gates bypassed, and writes the outcome back.
/// </para>
/// <para>
...
/// </para>
/// </summary>
public sealed class SqlDenialDatabaseRerunRepository : IDenialDatabaseRerunRepository
{
    private readonly string _connectionString;
    private readonly ILogger<SqlDenialDatabaseRerunRepository> _logger;

    public SqlDenialDatabaseRerunRepository(
        IConfiguration configuration, ILogger<SqlDenialDatabaseRerunRepository> logger)
    {
    ...
    public async Task<DenialDatabaseRerunSubmitResult> RequestAsync(
        IReadOnlyList<DenialDatabaseRerunLabSelection> labs,
        DenialDatabaseRerunContext context,
        CancellationToken ct)
    {
        var result = new DenialDatabaseRerunSubmitResult { BatchId = Guid.NewGuid() };
        ...
        const string sql = @"
IF EXISTS (SELECT 1 FROM dbo.DenialDatabaseRerunRequest
           WHERE LabId = @LabId AND Status IN ('Pending', 'Claimed'))
BEGIN
    SELECT CAST(0 AS BIT) AS Queued,
           (SELECT TOP (1) Status FROM dbo.DenialDatabaseRerunRequest
            WHERE LabId = @LabId AND Status IN ('Pending','Claimed')
            ORDER BY RerunRequestId DESC) AS ExistingStatus;
END
ELSE
BEGIN
    INSERT INTO dbo.DenialDatabaseRerunRequest
...
END";
        ...
        foreach (var lab in labs)
        {
            ...
            {
                _logger.LogError(ex, "Could not queue a denial database re-run for lab {LabId}.", lab.LabId);
    ...
    public async Task<IReadOnlyList<DenialDatabaseRerunRow>> GetRecentAsync(int take, CancellationToken ct)
    {
        const string sql = @"
SELECT TOP (@Take)
...
FROM   dbo.DenialDatabaseRerunRequest
ORDER  BY RequestedOn DESC, RerunRequestId DESC;";

        var rows = new List<DenialDatabaseRerunRow>();
        ...
        {
            rows.Add(new DenialDatabaseRerunRow
            {
