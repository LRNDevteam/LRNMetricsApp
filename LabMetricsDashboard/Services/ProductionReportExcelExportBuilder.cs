...
/// <param name="Rows">The data rows for this sheet.</param>
public sealed record RawDataSegment(string SheetName, List<Dictionary<string, object?>> Rows);
...
public static class ProductionReportExcelExportBuilder
{
        ...
        ILogger? logger = null)
    {
        ...
        {
            sw.Restart();
            BuildRawDataSheet(wb, seg.SheetName, seg.Rows, labName, ExcelTheme.TabBlue);
            claimSheetIdx++;
            logger?.LogInformation(
    ...
    public static XLWorkbook CreateWorkbook(
        ...
        ILogger? logger = null)
    {
        var claimSegments = PreSplitRawData("ClaimLevel", claimRows, logger);
        var lineSegments  = PreSplitRawData("LineLevel",  lineRows, logger);
    ...
    private static void BuildPayerBreakdownSheet(XLWorkbook wb, ProductionReportViewModel vm)
    {
        ...
        int hRow1 = row;
        WriteMergedHeader(ws, hRow1, hRow1 + 2, 1, 1, "Payer", ExcelTheme.BlueHeaderBg);
        int hCol = 2;
        foreach (var year in pbYears)
        {
        ...
        WriteMergedHeader(ws, hRow1, hRow1, hCol, hCol + metrics - 1, "Grand Total", ExcelTheme.AmberDarkBg);
        ...
        foreach (var year in pbYears)
        {
            ...
            foreach (var _ in mons)
            {
                ...
                WriteHeaderCell(ws, hRow3, hCol++, "Charge Amount", ExcelTheme.BlueSubHeaderBg);
            }
            WriteHeaderCell(ws, hRow3, hCol++, "No. of Claims", ExcelTheme.AmberHeaderBg);
            WriteHeaderCell(ws, hRow3, hCol++, "Charge Amount", ExcelTheme.AmberHeaderBg);
        }
        WriteHeaderCell(ws, hRow3, hCol++, "No. of Claims", ExcelTheme.AmberDarkBg);
        WriteHeaderCell(ws, hRow3, hCol, "Charge Amount", ExcelTheme.AmberDarkBg);
    ...
    private static void BuildPanelBreakdownSheet(XLWorkbook wb, ProductionReportViewModel vm)
    {
        ...
        int hRow1 = row;
        WriteMergedHeader(ws, hRow1, hRow1 + 2, 1, 1, "Panel", ExcelTheme.BlueHeaderBg);
        int hCol = 2;
        foreach (var year in years)
        {
            var mons = monthsByYear.GetValueOrDefault(year, []);
        ...
        {
            int col = 1;
            WriteCell(ws, row, col++, panel.PayerName, ExcelTheme.BlueGroupRowBg, isText: true);
            foreach (var year in years)
            {
                var mons = monthsByYear.GetValueOrDefault(year, []);
                foreach (var mk in mons)
                {
                    WriteCell(ws, row, col++, panel.ByMonth.GetValueOrDefault(mk, 0), ExcelTheme.BlueGroupRowBg);
                    WriteCurrencyCell(ws, row, col++, panel.ByMonthCharges.GetValueOrDefault(mk, 0m), ExcelTheme.BlueGroupRowBg);
                }
                WriteCell(ws, row, col++, panel.ByYear.GetValueOrDefault(year, 0), ExcelTheme.BlueGroupRowBg);
                WriteCurrencyCell(ws, row, col++, panel.ByYearCharges.GetValueOrDefault(year, 0m), ExcelTheme.BlueGroupRowBg);
            }
            WriteCell(ws, row, col++, panel.GrandTotal, ExcelTheme.BlueGroupRowBg);
            WriteCurrencyCell(ws, row, col, panel.GrandTotalCharges, ExcelTheme.BlueGroupRowBg);
            ws.Row(row).Style.Font.Bold = true;
            row++;
            ...
            {
                var bg = ExcelTheme.GetBlueRowBg(payerIdx);
                col = 1;
                WriteCell(ws, row, col++, $"    {payer.PayerName}", bg, isText: true);
                foreach (var year in years)
                {
    ...
    private static void BuildPayerPanelSheet(XLWorkbook wb, ProductionReportViewModel vm)
    {
        ...
        {
            WriteMergedHeader(ws, hRow1, hRow1, hCol, hCol + 1, panel, ExcelTheme.BlueHeaderBg);
            hCol += 2;
        }
    ...
    private static void BuildCptBreakdownSheet(XLWorkbook wb, ProductionReportViewModel vm)
    {
        ...
        // Header Row 3: Count of CPT (client report) — never SUM(Units)
        var cptUnitsHeader = !string.IsNullOrWhiteSpace(vm.CptUnitsLabel)
            ? vm.CptUnitsLabel
            : "Count of CPT";
        int hRow3 = hRow1 + 2;
        hCol = 2;
        foreach (var year in cptYears)
        {
            ...
            foreach (var _ in mons)
            {
                WriteHeaderCell(ws, hRow3, hCol++, cptUnitsHeader, ExcelTheme.BlueSubHeaderBg);
                WriteHeaderCell(ws, hRow3, hCol++, "Billed Amount", ExcelTheme.BlueSubHeaderBg);
            }
            WriteHeaderCell(ws, hRow3, hCol++, cptUnitsHeader, ExcelTheme.AmberHeaderBg);
            WriteHeaderCell(ws, hRow3, hCol++, "Billed Amount", ExcelTheme.AmberHeaderBg);
        }
        WriteHeaderCell(ws, hRow3, hCol++, cptUnitsHeader, ExcelTheme.AmberDarkBg);
        ...
        void WriteCptLine(CptBreakdownRow cptRow, XLColor bg)
        {
            ...
            foreach (var year in cptYears)
            {
                ...
                foreach (var mk in mons)
                {
                    ...
                    // Prefer Count of CPT (ClaimCount); fall back to Units only if count missing.
                    var countVal = cell.ClaimCount > 0 ? cell.ClaimCount : (double)cell.Units;
                    WriteDecimalCell(ws, row, col++, (decimal)countVal, bg);
                    WriteCurrencyCell(ws, row, col++, cell.BilledCharges, bg);
                }
                var yt = GetCptCell(cptRow.ByYear, year);
                var ytCount = yt.ClaimCount > 0 ? yt.ClaimCount : (double)yt.Units;
                WriteDecimalCell(ws, row, col++, (decimal)ytCount, bg);
                WriteCurrencyCell(ws, row, col++, yt.BilledCharges, bg);
            }
            var gtCount = cptRow.GrandTotalClaims > 0 ? cptRow.GrandTotalClaims : (double)cptRow.GrandTotalUnits;
            WriteDecimalCell(ws, row, col++, (decimal)gtCount, bg);
        ...
        foreach (var year in cptYears)
        {
            ...
            foreach (var mk in mons)
            {
                ...
                var countVal = cell.ClaimCount > 0 ? cell.ClaimCount : cell.Units;
                ws.Cell(row, gtCol).Value = countVal;
                ws.Cell(row, gtCol++).Style.NumberFormat.Format = "#,##0";
                ws.Cell(row, gtCol).Value = cell.BilledCharges;
                ws.Cell(row, gtCol++).Style.NumberFormat.Format = ExcelTheme.AccountingNumberFormat;
            }
            decimal yUnits = vm.CptBreakdownGrandByMonth.Where(kv => kv.Key.StartsWith($"{year:D4}"))
                .Sum(kv => kv.Value.ClaimCount > 0 ? kv.Value.ClaimCount : kv.Value.Units);
        ...
        decimal cptGrandUnits = vm.CptBreakdownGrandByMonth.Where(kv => int.Parse(kv.Key[..4]) > 1900)
            .Sum(kv => kv.Value.ClaimCount > 0 ? kv.Value.ClaimCount : kv.Value.Units);
    ...
    {
        return TryGetSplitDate(row, out var date) ? date.Year : 0;
    }

    private static int GetFirstBillDateMonth(Dictionary<string, object?> row)
    {
        return TryGetSplitDate(row, out var date) ? date.Month : 1;
    }

    private static bool TryGetSplitDate(Dictionary<string, object?> row, out DateTime date)
    {
