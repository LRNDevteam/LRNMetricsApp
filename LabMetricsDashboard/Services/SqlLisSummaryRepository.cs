...
public sealed class SqlLisSummaryRepository : ILisSummaryRepository
{
		...
		int totalSamples)
	{
		...
		// Read the self-pay total off the summary rows too. Scanning the raw groups instead counted a
		// sample whenever ANY ONE of three fields said self pay, so the card double-counted and could
		// not agree with the Self Pay row the table shows right below it (5852 on the card against
		// 3763 in the table). The raw scan stays only as a fallback for a layout with no self-pay row.
	...
	/// <summary>
	/// The Self Pay total as the summary table itself reports it, or null when the layout has no
	/// self-pay row to read.
	/// </summary>
	private static int? FindSelfPaySummaryTotal(List<LisSummaryRow> rows)
	{
		...
		if (sections.Count == 0)
		{
			...
			// The dynamic layout used by labs without a template repeats self pay under both the
			// bill-to and the final-status groupings; adding both would double the count.
