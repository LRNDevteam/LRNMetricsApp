/* =====================================================================
   Augustus — Claim/Line details SPs (lab-specific SELECT list)
...
BEGIN
    ...
    /* ==== Augustus ClaimLevel columns (Sql/Select_Script) — add/remove fields here ==== */
    SELECT
            [LabID],
            [LabName],
...
BEGIN
    ...
    /* ==== Augustus LineLevel columns (Sql/Select_Script) — add/remove fields here ==== */
    SELECT
            [LabID],
            [LabName],
            CASE WHEN [ClaimID] LIKE '%.00' THEN LEFT([ClaimID], LEN([ClaimID])-3) ELSE ISNULL(LTRIM(RTRIM([ClaimID])),'') END AS [ClaimID],
            ...
            [PatientName],
            [SubscriberId],
            ISNULL(TRY_CAST([ClaimAmount] AS DECIMAL(18,2)), 0) AS [ClaimAmount],
            [Date],
            [EnteredStatus],
            [BilledStatus],
            [CptWithUnits],
            [Proc],
            [CheqNo],
            ISNULL(TRY_CAST([AdjAmount] AS DECIMAL(18,2)), 0) AS [AdjAmount],
            ISNULL(TRY_CAST([InsBalance] AS DECIMAL(18,2)), 0) AS [InsBalance],
            ISNULL(TRY_CAST([PatBalance] AS DECIMAL(18,2)), 0) AS [PatBalance],
            [UpdatedDenial],
            [CombinedDenial],
            [PaymentPercent],
            [Loc],
            ...
            ISNULL(TRY_CAST([InsuranceBalance_Decimal] AS DECIMAL(18,2)), 0) AS [InsuranceBalance_Decimal],
            [ICDLineLevel],
            ISNULL(TRY_CAST([ClaimAmountRaw] AS DECIMAL(18,2)), 0) AS [ClaimAmountRaw]
