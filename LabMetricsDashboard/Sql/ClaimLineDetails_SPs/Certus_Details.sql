/* =====================================================================
   Certus — Claim/Line details SPs (lab-specific SELECT list)
...
BEGIN
    ...
    /* ==== Certus ClaimLevel columns (Sql/Select_Script) — add/remove fields here ==== */
    SELECT
            [LabID],
            [LabName],
...
BEGIN
    ...
    /* ==== Certus LineLevel columns (Sql/Select_Script) — add/remove fields here ==== */
    SELECT
            [LabID],
            [LabName],
            CASE WHEN [ClaimID] LIKE '%.00' THEN LEFT([ClaimID], LEN([ClaimID])-3) ELSE ISNULL(LTRIM(RTRIM([ClaimID])),'') END AS [ClaimID],
            ...
            [Description],
            [PostedWeek],
            ISNULL(TRY_CAST([BilledAmounts] AS DECIMAL(18,2)), 0) AS [BilledAmounts],
            [OriginalDenialCode],
            [DenialCombination],
            [PaymentPercent],
            [LineLevelUID],
