/* =====================================================================
   RisingTides — Claim/Line details SPs (lab-specific SELECT list)
...
BEGIN
    ...
    /* ==== RisingTides ClaimLevel columns (Sql/Select_Script) — add/remove fields here ==== */
    SELECT
            [LabID],
            [LabName],
...
BEGIN
    ...
    /* ==== RisingTides LineLevel columns (Sql/Select_Script) — add/remove fields here ==== */
    SELECT
            [LabID],
            [LabName],
            CASE WHEN [ClaimID] LIKE '%.00' THEN LEFT([ClaimID], LEN([ClaimID])-3) ELSE ISNULL(LTRIM(RTRIM([ClaimID])),'') END AS [ClaimID],
