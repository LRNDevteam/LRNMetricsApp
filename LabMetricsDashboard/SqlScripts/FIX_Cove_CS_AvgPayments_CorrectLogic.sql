...
-- Date: DATEADD(MONTH, -N, billed week-range / CheckDate end) — not DATEADD(DAY, -180)
-- Counts: COUNT(*)
-- ============================================================
SET NOCOUNT ON;
GO
...
BEGIN
    ...
    PRINT 'usp_RefreshCove_CS_AvgPayments — '
...
BEGIN
    ...
    DECLARE @EndDate DATE = ISNULL(@CheckDateTo,
        (SELECT MAX(TRY_CAST(CheckDate AS DATE))
         FROM dbo.ClaimLevelData
         WHERE TRY_CAST(CheckDate AS DATE) IS NOT NULL));

    -- Calendar months from week-range / CheckDate end — not DATEADD(DAY,-180).
    DECLARE @Cutoff DATE = DATEADD(MONTH, -@LastMonths, @EndDate);
    ...
    (
        ...
        WHERE TRY_CAST(CheckDate AS DATE) IS NOT NULL
          AND TRY_CAST(CheckDate AS DATE) >= @Cutoff
          AND TRY_CAST(CheckDate AS DATE) <= @EndDate
          AND NULLIF(LTRIM(RTRIM(Panelname)), '') IS NOT NULL
...
PRINT 'Refreshing Avg Payments snapshot...';
...
DECLARE @C3 DATE = DATEADD(MONTH, -3, @MaxCheck);
