/* ============================================================================
   Demo lab step 3 - re-stamp the copied data as LRNDemo instead of Cove.
...
/* No USE - Azure SQL Database cannot switch database context. Connect directly
   to LRNDemoLab. The DB_NAME() guard below is the only protection left. */

SET NOCOUNT ON;

DECLARE @Apply BIT = 0;   -- 0 = report only, 1 = actually re-stamp

DECLARE @OldLabId   INT           = 4;               -- Cove's id, inherited by the copy
DECLARE @NewLabId   INT           = 98;              -- must match dbo.Labs / dbo.LRNMetricsLab / appsettings LabsID
DECLARE @NewLabName NVARCHAR(200) = N'LRNDemo';       -- must match the lab config key and the JSON file name
...
PRINT N'Next: run Demo_LRNDemo_04_RegisterLab.sql against LRNMaster.';
GO
