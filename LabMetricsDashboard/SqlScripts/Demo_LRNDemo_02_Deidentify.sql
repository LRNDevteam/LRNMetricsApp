/* ============================================================================
   Demo lab step 2 of 4 - remove patient identifiers from the LRNDemoLab copy.
   Target: Azure SQL Database. Source data: CoveLRN (Azure SQL Managed Instance).
...
/* NO "USE" STATEMENT HERE - deliberately, and this is the one real difference
   from the on-prem version of this script.

   Azure SQL Database has no cross-database context: "USE <other database>" fails
   with "USE statement is not supported to switch between databases". So the
   connection itself has to be opened against LRNDemoLab:

       SSMS     Connect > Options > Connect to database: LRNDemoLab
       sqlcmd   -d LRNDemoLab

   That removes the safety net the on-prem version had, where the USE line pinned
   the target regardless of the connection. The DB_NAME() guard below is now the
   ONLY thing standing between this script and a live client database, so check
   what you are connected to before running with @Apply = 1. */
