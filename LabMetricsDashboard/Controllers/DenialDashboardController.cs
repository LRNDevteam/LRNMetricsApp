...
public class DenialDashboardController : Controller
{
    ...
    public async Task<IActionResult> Index([FromQuery] DenialDashboardFilters filters, CancellationToken cancellationToken)
    {
        ...
        {
            selectedLabId = labs.First().LabId;
        }
        ...
        var lineItemPage = Math.Clamp(normalizedFilters.LineItemPage <= 0 ? 1 : normalizedFilters.LineItemPage, 1, lineItemTotalPages);
    ...
    public async Task<IActionResult> ExportToExcel([FromQuery] DenialDashboardFilters filters, CancellationToken cancellationToken)
    {
        ...
        using var workbook = DenialDashboardExcelExportBuilder.CreateWorkbook(lineItems, filteredRecords);
    ...
    private static DenialDashboardFilters Normalize(DenialDashboardFilters filters, int? selectedLabId)
    {
        ...
        {
            LabId = selectedLabId,
            Status = NormalizeChoice(filters.Status),
            Priority = NormalizeChoice(filters.Priority),
            ActionCategory = NormalizeChoice(filters.ActionCategory),
            Deadline = NormalizeChoice(filters.Deadline),
            Classification = NormalizeChoice(filters.Classification),
            SalesRepname = NormalizeText(filters.SalesRepname),
            ClinicName = NormalizeText(filters.ClinicName),
            ReferringProvider = NormalizeText(filters.ReferringProvider),
            PayerName = NormalizeText(filters.PayerName),
            PayerType = NormalizeText(filters.PayerType),
            PanelName = NormalizeText(filters.PanelName),
    ...
    private static string NormalizeChoice(string? value) => string.IsNullOrWhiteSpace(value) ? "(All)" : value.Trim();
    ...
    {
        var query = records;
        if (filters.Status != "(All)") query = query.Where(x => EqualsIgnoreCase(x.Status, filters.Status));
        if (filters.Priority != "(All)") query = query.Where(x => EqualsIgnoreCase(x.Priority, filters.Priority));
        if (filters.ActionCategory != "(All)") query = query.Where(x => EqualsIgnoreCase(x.EffectiveActionCategory, filters.ActionCategory));
        if (filters.Classification != "(All)") query = query.Where(x => EqualsIgnoreCase(x.DenialClassification, filters.Classification));
        if (filters.Deadline != "(All)") query = query.Where(x => IsInDeadlineBucket(x, filters.Deadline));

        if (!string.IsNullOrWhiteSpace(filters.SalesRepname)) query = query.Where(x => ContainsIgnoreCase(x.SalesRepname, filters.SalesRepname));
        if (!string.IsNullOrWhiteSpace(filters.ClinicName)) query = query.Where(x => ContainsIgnoreCase(x.ClinicName, filters.ClinicName));
        if (!string.IsNullOrWhiteSpace(filters.ReferringProvider)) query = query.Where(x => ContainsIgnoreCase(x.ReferringProvider, filters.ReferringProvider));
        if (!string.IsNullOrWhiteSpace(filters.PayerName)) query = query.Where(x => ContainsIgnoreCase(x.PayerName, filters.PayerName));
        if (!string.IsNullOrWhiteSpace(filters.PayerType)) query = query.Where(x => ContainsIgnoreCase(x.PayerType, filters.PayerType));
        if (!string.IsNullOrWhiteSpace(filters.PanelName)) query = query.Where(x => ContainsIgnoreCase(x.PanelName, filters.PanelName));
    ...
    private static bool HasNoAdditionalFilters(DenialDashboardFilters filters)
    {
        return filters.Status == "(All)"
            ...
            && string.IsNullOrWhiteSpace(filters.SalesRepname)
            && string.IsNullOrWhiteSpace(filters.ClinicName)
            && string.IsNullOrWhiteSpace(filters.ReferringProvider)
            && string.IsNullOrWhiteSpace(filters.PayerName)
            && string.IsNullOrWhiteSpace(filters.PayerType)
            && string.IsNullOrWhiteSpace(filters.PanelName)
